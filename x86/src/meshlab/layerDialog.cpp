    ...
    : QDockWidget(parent)
{
	...
	// Remove titlebar by Johnny Xu, 2017/7/14
    ...
    {
		// Fix flash across window while startup by Johnny Xu, 2017/6/28.
 //       _tabw->setVisible(true);

        groupboxlay->addWidget(_tabw);
    }

	_applytovis = new QCheckBox(QTextCodec::codecForName("GBK")->toUnicode("应用到所有可见层") /*QString("apply to all visible layers")*/, this);
	...
	// Fix flash across window while startup by Johnny Xu, 2017/6/28.
	setMinimumSize(minimumSize().width() + 10, minimumSize().height());
...
void LayerDialog::paintEvent(QPaintEvent*)
{
...
void LayerDialog::showContextMenu(const QPoint& pos)
{
    ...
    if (sigsender == ui->meshTreeWidget)
    {
        ...
        if(mItem)
        {
			...
			// Fix right menu issue in titlebar custom situation by Johnny Xu, 2017/7/11
            foreach (QWidget *widget, QApplication::allWidgets() /*QApplication::topLevelWidgets()*/)
            {
...
//     QString preWarn    = "<font face=\"courier\" size=3 color=\"red\">" + QTextCodec::codecForName("GBK")->toUnicode("警告: ") /* Warning: "*/ ;
   ...
   updateProjectName(md->docLabel());
	...
	foreach(MeshModel* mmd, md->meshList)
	{
        ...
        if (rdit != dtf.end())
        {
			...
			// Delete sub-tree node information by Johnny Xu, 2017/6/28
...
void LayerDialog::addDefaultNotes(QTreeWidgetItem * parent, MeshModel *meshModel)
{
    ...
    fileItem->setText(2, QTextCodec::codecForName("GBK")->toUnicode("文件") /*QString("File")*/);
    ...
    vertItem->setText(2, QTextCodec::codecForName("GBK")->toUnicode("顶点") /*QString("Vertices")*/);
    ...
    if(meshModel->cm.en>0)
	{
        ...
        edgeItem->setText(2, QTextCodec::codecForName("GBK")->toUnicode("边缘") /*QString("Edges")*/);
    ...
    faceItem->setText(2, QTextCodec::codecForName("GBK")->toUnicode("面") /*QString("Faces")*/);
    ...
    {
        QTreeWidgetItem *vertItem = new QTreeWidgetItem();
        vertItem->setText(2, QTextCodec::codecForName("GBK")->toUnicode("顶点属性") /*QString("Vert Attr.")*/);
    ...
    {
        QTreeWidgetItem *vertItem = new QTreeWidgetItem();
        vertItem->setText(2, QTextCodec::codecForName("GBK")->toUnicode("面属性") /*QString("Face Attr.")*/);
        vertItem->setText(3, "float "+QString(AttribNameVector[i].c_str()));
        parent->addChild(vertItem);
        updateColumnNumber(vertItem);
    }
    ...
    {
        QTreeWidgetItem *vertItem = new QTreeWidgetItem();
        vertItem->setText(2, QTextCodec::codecForName("GBK")->toUnicode("顶点属性") /*QString("Face Attr.")*/);
        vertItem->setText(3, "Point3f "+QString(AttribNameVector[i].c_str()));
        parent->addChild(vertItem);
        updateColumnNumber(vertItem);
    }
...
void LayerDialog::updateDecoratorParsView()
{
    ...
    for(int ii = 0; ii < decList.size(); ++ii)
    {
        ...
        if (!decPlug)
        {
            mw->GLA()->Logf(GLLogStream::SYSTEM, "错误: 一个装饰插件非正常工作.");
            return;
        }
        else
        {
    ...
    :QFrame(parent),mainWin(mw),frame(NULL),savebut(NULL),resetbut(NULL),loadbut(NULL),dialoglayout(NULL)
{
    ...
    else
    {
        ...
        if (tmpSet.paramList.size() != 0)
        {
            ...
            savebut = new QPushButton(QTextCodec::codecForName("GBK")->toUnicode("保存") /*"Save"*/, parent);
            resetbut = new QPushButton(QTextCodec::codecForName("GBK")->toUnicode("重置") /*"Reset"*/, parent);
            loadbut = new QPushButton(QTextCodec::codecForName("GBK")->toUnicode("加载") /*"Load"*/, parent);
            ...
            savebut ->setMaximumSize(savebut ->maximumSize().width(), savebut ->minimumSizeHint().height());
            resetbut->setMaximumSize(resetbut->maximumSize().width(), resetbut->minimumSizeHint().height());
            loadbut ->setMaximumSize(loadbut ->maximumSize().width(), loadbut ->minimumSizeHint().height());
