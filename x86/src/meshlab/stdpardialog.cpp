...

{
	...
	fieldDesc = new QLabel(rp->pd->fieldDesc + QTextCodec::codecForName("GBK")->toUnicode(" (绝对值和百分比)") /*" (abs and %)"*/,this);
	...
	QLabel *absLab = new QLabel("<i> <small>" + QTextCodec::codecForName("GBK")->toUnicode("世界坐标")/* world unit*/ + "</small></i>");
	QLabel *percLab = new QLabel("<i> <small>" + QTextCodec::codecForName("GBK")->toUnicode("perc在")/* perc on"*/ + QString("(%1 .. %2)").arg(m_min).arg(m_max) + "</small></i>");
...
Point3fWidget::Point3fWidget(QWidget *p, RichPoint3f* rpf, QWidget *gla_curr): MeshLabWidget(p,rpf)
{
	...
	{
		getPoint3Button = new QPushButton(QTextCodec::codecForName("GBK")->toUnicode("获取"),this);
...
Matrix44fWidget::Matrix44fWidget(QWidget *p, RichMatrix44f* rpf,  QWidget *gla_curr): MeshLabWidget(p,rpf)
{
	...
	QPushButton     * getMatrixButton = new QPushButton(QTextCodec::codecForName("GBK")->toUnicode("从当前层读取") /*"Read from current layer"*/);
	vlay->addWidget(getMatrixButton);

	QPushButton     * pasteMatrixButton = new QPushButton(QTextCodec::codecForName("GBK")->toUnicode("粘贴") /*"Paste from clipboard"*/);
...
void ShotfWidget::getShot()
{
	...
	case 3:
		{
			QString filename = QFileDialog::getOpenFileName(this, QTextCodec::codecForName("GBK")->toUnicode("加载相机") /*tr("Load xml camera")*/, "./", tr("Xml Files (*.xml)"));
			QFile qf(filename);
...
{
	pickcol = QColorDialog::getColor(pickcol, this->parentWidget(), QTextCodec::codecForName("GBK")->toUnicode("选择颜色")/*"Pick a Color"*/, QColorDialog::DontUseNativeDialog|QColorDialog::ShowAlphaChannel);
	if(pickcol.isValid())
	{
		collectWidgetValue();
		updateColorInfo(ColorValue(pickcol));

		// Fix cancel issue. Moved by Johnny Xu, 2017/2/23
		emit dialogParamChanged();
	}
}
...
void OpenFileWidget::selectFile()
{
	...
	QString ext;
	fl = QFileDialog::getOpenFileName(this, QTextCodec::codecForName("GBK")->toUnicode("打开") /*tr("Open")*/, dec->defVal->getFileName(), dec->exts.join(" "));
	collectWidgetValue();
	updateFileName(fl);
...
void SaveFileWidget::selectFile()
{
	...
	QString ext;
	fl = QFileDialog::getSaveFileName(this, QTextCodec::codecForName("GBK")->toUnicode("保存") /*tr("Save")*/, dec->defVal->getFileName(), dec->ext);
	collectWidgetValue();
	updateFileName(fl);
	FileValue fileName(fl);
	rp->pd->defVal->set(fileName);
