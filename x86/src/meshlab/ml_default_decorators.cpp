    ...
    if (opts._peredge_extra_enabled)
    {
        if (opts._peredge_edgeboundary_enabled || opts._peredge_faceboundary_enabled)
        {
            ...
            if (opts._peredge_edgeboundary_enabled)
            {
                QString inf;
                if(m.cm.fn==0) 
                    inf += "<b>" + QString::number(bvH().size()) + " </b>" + QTextCodec::codecForName("GBK")->toUnicode("个顶点");
                else
                    inf += "<b>" + QString::number(beH().size()/2) + " </b>" + QTextCodec::codecForName("GBK")->toUnicode("个边");
                log.RealTimeLog(QTextCodec::codecForName("GBK")->toUnicode("边界") /*"Boundary"*/, m.shortName(), inf);
            }
            ...
            if (opts._peredge_faceboundary_enabled)
            {
                ...
                if(m.cm.fn==0) 
                    ...
                    inf += "<b>" + QString::number(bfH().size()/3) + " </b>" + QTextCodec::codecForName("GBK")->toUnicode("个面");
                log.RealTimeLog(QTextCodec::codecForName("GBK")->toUnicode("边界面")/*"Boundary Faces"*/, m.shortName(), inf);
            }      
        }
        ...
        if (opts._peredge_vertmanifold_enabled)
        {
              ...
              QString inf;
              inf += "<b>" + QString::number(vvH().size()) + " </b>" +  QTextCodec::codecForName("GBK")->toUnicode("个非流型顶点") + "<br><b>" + QString::number(tvH().size() / 3) + "</b>" + QTextCodec::codecForName("GBK")->toUnicode("个非流型顶点上的面");
              log.RealTimeLog(QTextCodec::codecForName("GBK")->toUnicode("非流型顶点") /*"Non Manifold Vertices"*/, m.shortName(), inf);
        }
        ...
        if (opts._peredge_edgemanifold_enabled)
        {
            ...
            QString inf;
            inf += "<b>" + QString::number(bvH().size()/2) + " </b>" + QTextCodec::codecForName("GBK")->toUnicode("个非流型边") + "<br><b>" + QString::number(fvH().size()/3) + "</b>" + QTextCodec::codecForName("GBK")->toUnicode("个非流型边上的面");
            log.RealTimeLog(QTextCodec::codecForName("GBK")->toUnicode("非流型边"), m.shortName(), inf);
        }
