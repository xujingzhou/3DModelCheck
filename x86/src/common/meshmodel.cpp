...
QStringList MeshDocument::getModifiedMeshLabel()
{
	QStringList modifiedMeshLabels;
    int i = 0;
    while(i < meshList.size())
    {
        if (meshList[i]->meshModified())
			modifiedMeshLabels.append(meshList[i]->label());
        ++i;
    }

    return modifiedMeshLabels;
}
