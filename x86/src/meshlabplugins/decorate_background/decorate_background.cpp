...
QString DecorateBackgroundPlugin::decorationName(FilterIDType id) const
{
    switch (id)
    {
		case DP_SHOW_CUBEMAPPED_ENV:  
			return tr("Cube mapped background");
		case DP_SHOW_GRID:            
			return QTextCodec::codecForName("GBK")->toUnicode("背景网格") /*tr("Background Grid")*/;
    }
...
QString DecorateBackgroundPlugin::decorationInfo(FilterIDType id) const
{
    ...
    {
		case DP_SHOW_CUBEMAPPED_ENV :  
			return tr("Draws a customizable cube mapped background that is sync with trackball rotation");
		case DP_SHOW_GRID :            
			return "" /*tr("Draws a gridded background that can be used as a reference.")*/;
    }
    assert(0);
    return QString();
}
...
void DecorateBackgroundPlugin::initGlobalParameterSet(QAction *action, RichParameterSet &parset)
{
    switch(ID(action))
    {
		...
		case DP_SHOW_GRID :
		{
			parset.addParam(new RichFloat(BoxRatioParam(), 1.2, QTextCodec::codecForName("GBK")->toUnicode("边框比例:"), "" /*"Box Ratio", "The size of the grid around the object w.r.t. the bbox of the object"*/));
			parset.addParam(new RichFloat(GridMajorParam(), 10, QTextCodec::codecForName("GBK")->toUnicode("主间隙:"), "" /*"Major Spacing", ""*/));
			parset.addParam(new RichFloat(GridMinorParam(), 1, QTextCodec::codecForName("GBK")->toUnicode("次间隙:"), "" /*"Minor Spacing", ""*/));
			parset.addParam(new RichBool(GridBackParam(), true, QTextCodec::codecForName("GBK")->toUnicode("剔除前面网格"), "" /*"Front grid culling", ""*/));
			parset.addParam(new RichBool(ShowShadowParam(), false, QTextCodec::codecForName("GBK")->toUnicode("显示投影"), "" /*"Show silhouette", ""*/));
			parset.addParam(new RichColor(GridColorBackParam(), QColor(163,116,35,255), QTextCodec::codecForName("GBK")->toUnicode("后端网格色:"), "" /*"Back Grid Color", ""*/));
			parset.addParam(new RichColor(GridColorFrontParam(), QColor(22,139,119,255), QTextCodec::codecForName("GBK")->toUnicode("前端前景色:"), "" /*"Front grid Color", ""*/));
		}
			break;
    }
}
