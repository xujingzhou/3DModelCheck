  ...
  //string filename = fileName.toUtf8().data();
	...
	if (formatName.toUpper() == tr("3DS"))
	{
        ...
        if (!file)
        {
			...
			QString strBox = errorMsgFormat.arg(fileName, vcg::tri::io::Importer3DS<CMeshO>::ErrorMsg(result));
			QMessageBox box(QMessageBox::Warning, QString::fromLocal8Bit("3DS 打开错误"), strBox);
        ...
        if (!singleLayer)
        {
            ...
            for (p=file->nodes; p!=0; p=p->next, ++i)
            {
				...
				{
					QMessageBox box(QMessageBox::Warning, QString::fromLocal8Bit("3DS 打开错误"), missingTextureFilesMsg);
					box.setStandardButtons(QMessageBox::Ok);
					box.setButtonText(QMessageBox::Ok, QString::fromLocal8Bit("确 定"));
					box.exec();
				}
