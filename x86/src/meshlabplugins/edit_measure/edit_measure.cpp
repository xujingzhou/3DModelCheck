...
{
    return QTextCodec::codecForName("GBK")->toUnicode("在模型中测量两点间距离") /*tr("Allow to measure distances between points of a model")*/;
}
...
void EditMeasurePlugin::Decorate(MeshModel & m, GLArea * gla,QPainter* p)
{
	...
	if(rubberband.IsReady())
	{
		...
		this->Log(GLLogStream::FILTER, "距离 %s: %d mm", newM.ID.toStdString().c_str(), newM.length);
	}

	for (int mind = 0; mind<measures.size(); mind++)
	{
		rubberband.RenderLine(gla, measures[mind].startP, measures[mind].endP);
		// Add a temporary meature unit "mm" by Johnny Xu, 2017/2/28  -- The follow-up still need to research --
		vcg::glLabel::render(p, measures[mind].endP, QString("%1: %2 mm").arg(measures[mind].ID).arg(measures[mind].length));
	}

	QString instructions;
	instructions = QTextCodec::codecForName("GBK")->toUnicode("按C键清除"); /*"C to clear, P to print, S to save"*/;

	QString savedmeasure = "<br>";
	for (int mind = 0; mind<measures.size(); mind++)
	{
		savedmeasure.append(QString("%1 - %2 mm<br>").arg(measures[mind].ID).arg(measures[mind].length));
	}

	if (measures.size() == 0)
		this->RealTimeLog(QTextCodec::codecForName("GBK")->toUnicode("点到点的测量") /*"Point to Point Measure"*/, m.shortName(),
			" -- "
			);
	else
		this->RealTimeLog(QTextCodec::codecForName("GBK")->toUnicode("点到点的测量") /*"Point to Point Measure"*/, m.shortName(),
...
void EditMeasurePlugin::keyReleaseEvent(QKeyEvent *e, MeshModel &mod, GLArea *gla)
{
	...
	{
		this->Log(GLLogStream::FILTER, "------- 距离 -------");
		this->Log(GLLogStream::FILTER, "标识号: 距离 [A点坐标][B点坐标]" /*"ID: Dist [pointA][pointB]"*/);
		for (int mind = 0; mind<measures.size(); mind++)
		{
	...
	if (e->key() == Qt::Key_S) // save
	{
		...
		if (openFile.open(QIODevice::ReadWrite))
		{
			...
			this->Log(GLLogStream::WARNING, "已保存测量数据到文件: %s", qPrintable(saveFileName));
		}
		else
		{
			// Modified by Johnny Xu, 2017/2/20
			this->Log(GLLogStream::WARNING, "不能保存测量数据到文件: %s", qPrintable(saveFileName));
		}
	}
}
