...
{
	QString buf;
	cb(0, qPrintable(QTextCodec::codecForName("GBK")->toUnicode("开始处理黏合的网格: ") + QString("(%1/%2)\n").arg(gluedNum()).arg(nodeMap.size()) 
		/*buf.sprintf("Starting Processing of %i glued meshes out of %i meshes\n",gluedNum(),nodeMap.size())*/));

	/******* Occupancy Grid Computation *************/
	cb(0, qPrintable(QTextCodec::codecForName("GBK")->toUnicode("开始计算重叠黏合的网格: ") + QString("%1\n").arg(gluedNum()))) /*qPrintable(buf.sprintf("Computing Overlaps %i glued meshes...\n",gluedNum() ))*/;
	...
	cb(0, qPrintable(QTextCodec::codecForName("GBK")->toUnicode("好的重叠弧: ") + QString("(%1/%2)\n").arg(totalArcNum).arg(OG.SVA.size()) /* buf.sprintf("Arc with good overlap %6i (on  %6lu)\n",totalArcNum,OG.SVA.size())*/));
	cb(0, qPrintable(QString("%1").arg(preservedArcNum) + QTextCodec::codecForName("GBK")->toUnicode("个被保留，") + QString("%1").arg(recalcArcNum) + QTextCodec::codecForName("GBK")->toUnicode("个被重新计算。\n") /*buf.sprintf(" %6i preserved %i Recalc \n",preservedArcNum,recalcArcNum)*/)) ;
	for(size_t i=0;i<OG.SVA.size() && OG.SVA[i].norm_area > mtp.arcThreshold; ++i)
	{
		...
		if(curResult==0 || curResult->err >= percentileThr) // missing arc and arc with great error must be recomputed.
		{
			...
			if( curResult->IsValid() )
			{
				...
				cb(0, qPrintable(QString("(%1/%2) %3 -> %4").arg(int(i+1)).arg(totalArcNum).arg(OG.SVA[i].s).arg(OG.SVA[i].t) + QTextCodec::codecForName("GBK")->toUnicode("个被拼接。\n")
					/*buf.sprintf("(%3i/%3i) %2i -> %2i Aligned AvgErr dd=%f -> dd=%f \n",int(i+1),totalArcNum,OG.SVA[i].s,OG.SVA[i].t,dd.first,dd.second)*/));
			}
			else
			{
				cb(0, qPrintable(QString("(%1/%2) %3 -> %4").arg(int(i+1)).arg(totalArcNum).arg(OG.SVA[i].s).arg(OG.SVA[i].t) + QTextCodec::codecForName("GBK")->toUnicode("一个弧拼接失败: ") + QString("dd=%1 \n").arg(vcg::AlignPair::ErrorMsg(curResult->status))
					/* buf.sprintf( "(%3i/%3i) %2i -> %2i Failed Alignment of one arc %s\n",int(i+1),totalArcNum,OG.SVA[i].s,OG.SVA[i].t,vcg::AlignPair::ErrorMsg(curResult->status))*/));
				resultList.pop_back();
			}
		}
	}
	...
	if(totalArcNum==0) 
	{
		cb(0, qPrintable(QTextCodec::codecForName("GBK")->toUnicode("失败: 这儿没有重叠网格或者没有合适的拼接弧。\n\n")
	...
	if(resultList.empty()) 
	{
		cb(0, qPrintable(QTextCodec::codecForName("GBK")->toUnicode("失败: 在候选的弧中没有合适的拼接弧。\n\n")
	...
	cb(0, qPrintable(QTextCodec::codecForName("GBK")->toUnicode("成功完成了网格到网格的拼接。\n")
		/* buf.sprintf("Completed Mesh-Mesh Alignment: Avg Err %5.3f Median %5.3f 90\% %5.3f\n",H.Avg(), H.Percentile(0.5f), H.Percentile(0.9f))*/));
...
void MeshTree::ProcessGlobal(vcg::AlignPair::Param &ap)
{
	...
	cb(0, qPrintable(QTextCodec::codecForName("GBK")->toUnicode("已完成所有的拼接。\n\n")) /*qPrintable(buf.sprintf("Completed Global Alignment (error bound %6.4f)\n", StartGlobErr))*/);
