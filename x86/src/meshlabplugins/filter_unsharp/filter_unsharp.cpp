...
QString FilterUnsharp::filterName(FilterIDType filter) const
{
	switch(filter)
	{
		case FP_LAPLACIAN_SMOOTH :					
			return QTextCodec::codecForName("GBK")->toUnicode("拉普拉斯平滑") /*QString("Laplacian Smooth")*/;
		case FP_HC_LAPLACIAN_SMOOTH :			
			return QTextCodec::codecForName("GBK")->toUnicode("HC拉普拉斯平滑") /*QString("HC Laplacian Smooth")*/;
		case FP_SD_LAPLACIAN_SMOOTH :			
			return QString("ScaleDependent Laplacian Smooth");
		case FP_TWO_STEP_SMOOTH :	    		
			return QString("TwoStep Smooth");
		case FP_TAUBIN_SMOOTH :					
			return QTextCodec::codecForName("GBK")->toUnicode("Taubin平滑") /*QString("Taubin Smooth")*/;
		case FP_DEPTH_SMOOTH :					
			return QString("Depth Smooth");
		case FP_DIRECTIONAL_PRESERVATION :     
			return QString("Directional Geom. Preserv.");
		case FP_CREASE_CUT :			
			return QString("Cut mesh along crease edges");
		case FP_FACE_NORMAL_NORMALIZE:     
			return QString("Normalize Face Normals");
		case FP_VERTEX_NORMAL_NORMALIZE:	
			return QString("Normalize Vertex Normals");
		case FP_FACE_NORMAL_SMOOTHING:	
			return QString("Smooth Face Normals");
		case FP_VERTEX_QUALITY_SMOOTHING:	
			return QString("Smooth Vertex Quality");
		case FP_UNSHARP_NORMAL:			
			return QString("UnSharp Mask Normals");
		case FP_UNSHARP_GEOMETRY:         
			return QString("UnSharp Mask Geometry");
		case FP_UNSHARP_QUALITY:		
			return QString("UnSharp Mask Quality");
		case FP_UNSHARP_VERTEX_COLOR:	 
			return QString("UnSharp Mask Color");
		case FP_RECOMPUTE_VERTEX_NORMAL:
			return QString("Re-Compute Vertex Normals");
		case FP_RECOMPUTE_FACE_NORMAL:     
			return QString("Re-Compute Face Normals");
		case FP_RECOMPUTE_QUADFACE_NORMAL:	
			return QString("Re-Compute Per-Polygon Face Normals");
		case FP_LINEAR_MORPH :	
			return QString("Vertex Linear Morphing");
		case FP_SCALAR_HARMONIC_FIELD: 
			return QString("Generate Scalar Harmonic Field");
...
QString FilterUnsharp::filterInfo(FilterIDType filterId) const
{
	...
	{
		case FP_LAPLACIAN_SMOOTH :        
			return tr(""); /*tr("Laplacian smooth of the mesh: for each vertex it calculates the average position with nearest vertex");*/
		case FP_HC_LAPLACIAN_SMOOTH :     
			return tr(""); /*tr("HC Laplacian Smoothing, extended version of Laplacian Smoothing, based on the paper of Vollmer, Mencl, and Muller");*/
		case FP_SD_LAPLACIAN_SMOOTH :     
			return tr("Scale Dependent Laplacian Smoothing, extended version of Laplacian Smoothing, based on the Fujiwara extended umbrella operator");
		case FP_TWO_STEP_SMOOTH :         
			return tr("Two Step Smoothing, a feature preserving/enhancing fairing filter. It is based on a Normal Smoothing step where similar normals are averaged together and a step where the vertexes are fitted on the new normals. Based on: <br>"
		...
		case FP_TAUBIN_SMOOTH :         
			return tr(""); /*tr("The &lambda;-&mu; Taubin smoothing, it make two steps of smoothing, forth and back, for each iteration. Based on:<br>"
													"Gabriel Taubin,<br><b>A signal processing approach to fair surface design</b><br>Siggraph 1995" );*/
		case FP_DEPTH_SMOOTH :           
			return tr("A laplacian smooth that is constrained to move vertices only along the view direction.");
		case FP_DIRECTIONAL_PRESERVATION : 
			return tr("Store and Blend the current geometry with the result of another previous smoothing processing step. It is useful to limit the influence of any smoothing algorithm along the viewing direction. This is import to cope with the biased distribution of the error in many scanning devices. TOF scanner usually have very good <b>x,y</b> accuracy but suffer of great depth errors.");
		case FP_CREASE_CUT:                
			return tr("Cut the mesh along crease edges, duplicating the vertices as necessary. Crease edges are defined according to the variation of normal of the adjacent faces");
		case FP_FACE_NORMAL_NORMALIZE:    
			return tr("Normalize Face Normal Lenghts");
		case FP_VERTEX_NORMAL_NORMALIZE:   
			return tr("Normalize Vertex Normal Lenghts");
		case FP_VERTEX_QUALITY_SMOOTHING:	
			return tr("Laplacian smooth of the quality values.");
		case FP_FACE_NORMAL_SMOOTHING:    
			return tr("Smooth Face Normals without touching the position of the vertices.");
		case FP_UNSHARP_NORMAL:           
			return tr("Unsharp mask filtering of the normals, putting in more evidence normal variations");
		case FP_UNSHARP_GEOMETRY:      
			return tr("Unsharp mask filtering of geometric shape, putting in more evidence ridges and valleys variations");
		case FP_UNSHARP_QUALITY:       
			return tr("Unsharp mask filtering of the quality field");
		case FP_UNSHARP_VERTEX_COLOR:    
			return tr("Unsharp mask filtering of the color, putting in more evidence color edge variations");
		case FP_RECOMPUTE_VERTEX_NORMAL:  
			return tr("Recompute vertex normals according to four different schemes:<br>"
		...
		case FP_RECOMPUTE_FACE_NORMAL:      
			return tr("Recompute face normals as the normal of the plane of the face");
		case FP_RECOMPUTE_QUADFACE_NORMAL:
			return tr("Recompute face normals as the normal of the average of the normals of the triangles that builds a polygon. Useful for showing shaded quad or polygonal meshes represented using faux edges.");
		case FP_LINEAR_MORPH :            
			return tr("Morph current mesh towards a target with the same number of vertices. <br> The filter assumes that the two meshes have also the same vertex ordering.");
		case FP_SCALAR_HARMONIC_FIELD:
			return QString("Generates a scalar harmonic field over the mesh. Scalar values must be assigned to at least two vertices as Dirichlet boundary conditions. Applying the filter, a discrete Laplace operator generates the harmonic field values for all the mesh vertices. Note that the field values is stored in the quality per vertex attribute of the mesh\n\nFor more details see:\n Kai Xua, Hao Zhang, Daniel Cohen-Or, Yueshan Xionga,'Dynamic Harmonic Fields for Surface Processing'.\nin Computers & Graphics, 2009");
...
void FilterUnsharp::initParameterSet(QAction *action, MeshDocument &md, RichParameterSet &parlst)
{
	switch(ID(action))
	{
		...
		case FP_LAPLACIAN_SMOOTH:
			parlst.addParam(new RichInt  ("stepSmoothNum", (int) 3, QTextCodec::codecForName("GBK")->toUnicode("平滑次数:"), "" /*"Smoothing steps", "The number of times that the whole algorithm (normal smoothing + vertex fitting) is iterated."*/));
			parlst.addParam(new RichBool ("Boundary", true, QTextCodec::codecForName("GBK")->toUnicode("边界平滑"), "" /*"1D Boundary Smoothing", "If true the boundary edges are smoothed only by themselves (e.g. the polyline forming the boundary of the mesh is independently smoothed). Can reduce the shrinking on the border but can have strange effects on very small boundaries."*/));
			parlst.addParam(new RichBool ("cotangentWeight", true, QTextCodec::codecForName("GBK")->toUnicode("余切权重"), "" /*"Cotangent weighting", "If true the cotangente weighting scheme is computed for the averaging of the position. Otherwise (false) the simpler umbrella scheme (1 if the edge is present) is used."*/));
			parlst.addParam(new RichBool ("Selected", md.mm()->cm.sfn>0,  QTextCodec::codecForName("GBK")->toUnicode("仅影响选中的面"), "" /*"Affect only selected faces","If checked the filter is performed only on the selected faces"*/));
			break;
		...
		case FP_TAUBIN_SMOOTH:
			parlst.addParam(new RichFloat("lambda", (float) 0.5,  QTextCodec::codecForName("GBK")->toUnicode("Lambda:"), "" /*"Lambda", "The lambda parameter of the Taubin Smoothing algorithm"*/));
			parlst.addParam(new RichFloat("mu", (float) -0.53,  QTextCodec::codecForName("GBK")->toUnicode("mu:"), "" /*"mu", "The mu parameter of the Taubin Smoothing algorithm"*/));
			parlst.addParam(new RichInt  ("stepSmoothNum", (int) 10,  QTextCodec::codecForName("GBK")->toUnicode("平滑次数:"), "" /*"Smoothing steps", "The number of times that the taubin smoothing is iterated. Usually it requires a larger number of iteration than the classical laplacian"*/));
			parlst.addParam(new RichBool ("Selected", md.mm()->cm.sfn>0,  QTextCodec::codecForName("GBK")->toUnicode("仅影响选中的面"), "" /*"Affect only selected faces","If checked the filter is performed only on the selected faces"*/));
			break;
		...
		case FP_LAPLACIAN_SMOOTH :
			{
				...
				Log("平滑了%d个点", Selected>0 ? m.cm.svn : m.cm.vn);
...
MeshFilterInterface::FILTER_ARITY FilterUnsharp::filterArity( QAction * filter ) const
{
    ...
    {
		case FP_LAPLACIAN_SMOOTH :	
		// Comment code for not popup dialog in plungin. Modified by Johnny Xu, 2017/2/21 
//		case FP_HC_LAPLACIAN_SMOOTH :		
		case FP_SD_LAPLACIAN_SMOOTH :		
		case FP_TWO_STEP_SMOOTH :	    	
		case FP_TAUBIN_SMOOTH :				
