...
QString DecorateBasePlugin::decorationInfo(FilterIDType filter) const
{
	...
	{
	case DP_SHOW_AXIS:             
		return tr(""); /*tr("Draw XYZ axes in world coordinates")*/;
	case DP_SHOW_BOX_CORNERS:      
		return tr("Draw object's bounding box corners");
	case DP_SHOW_NORMALS:          
		return tr("Draw per vertex/face normals");
	case DP_SHOW_CURVATURE:       
		return tr("Draw per vertex/face principal curvature directions");
	case DP_SHOW_LABEL:           
		return tr("Draw on all the vertex/edge/face a label with their index<br> Useful for debugging<br>(WARNING: do not use it on large meshes)");
	case DP_SHOW_QUALITY_HISTOGRAM: 
		return tr("Draw a (colored) Histogram of the per vertex/face quality");
	case DP_SHOW_QUALITY_CONTOUR:  
		return tr("Draw quality contours, e.g. the isolines of the quality field defined over the surface ");
	case DP_SHOW_CAMERA:          
		return tr("Draw the position of the mesh camera and raster cameras");
	case DP_SHOW_TEXPARAM:        
		return tr("Draw an overlayed flattened version of the current mesh that show the current parametrization");
	case DP_SHOW_SELECTED_MESH:   
		return tr("Highlight the current mesh");
	}
...
QString DecorateBasePlugin::decorationName(FilterIDType filter) const
{
	...
	{
	case DP_SHOW_NORMALS:     
		return QString("Show Normal");
	case DP_SHOW_CURVATURE:   
		return QString("Show Curvature");
	case DP_SHOW_BOX_CORNERS:  
		return QString("Show Box Corners");
	case DP_SHOW_AXIS:         
		return QTextCodec::codecForName("GBK")->toUnicode("ÏÔÊ¾×ø±êÖá") /*QString("Show Axis")*/;
	case DP_SHOW_LABEL:       
		return QString("Show Labels");
	case DP_SHOW_CAMERA:       
		return QString("Show Camera");
	case DP_SHOW_TEXPARAM:     
		return QString("Show UV Tex Param");
	case DP_SHOW_QUALITY_HISTOGRAM: 
		return QString("Show Quality Histogram");
	case DP_SHOW_QUALITY_CONTOUR: 
		return QString("Show Quality Contour");
	case DP_SHOW_SELECTED_MESH:  
		return QString("Show Current Mesh");
	default:
		assert(0);
	}
...
void DecorateBasePlugin::decorateMesh(QAction *a, MeshModel &m, RichParameterSet *rm, GLArea *gla, QPainter *painter,GLLogStream &_log)
{
	...
	case DP_SHOW_TEXPARAM : 
		this->DrawTexParam(m,gla,painter,rm,qf); 
		break;
...
void DecorateBasePlugin::drawQuotedLine(const Point3d &a,const Point3d &b, float aVal, float bVal, float tickScalarDistance, QPainter *painter, QFont qf,float angle,bool rightAlign)
{
	...
	float firstTickTen;
	if(aVal > 0) 
		firstTickTen = aVal - fmod(aVal,tickDistTen) + tickDistTen;
	else 
		firstTickTen = aVal - fmod(aVal,tickDistTen);
...
int DecorateBasePlugin::getDecorationClass(QAction *action) const
{
	...
	case DP_SHOW_LABEL :
	case DP_SHOW_TEXPARAM : 
		return DecorateBasePlugin::PerMesh;
	case DP_SHOW_AXIS : 
		return DecorateBasePlugin::PerDocument;
	case DP_SHOW_CAMERA : 
		return DecorateBasePlugin::PerDocument;
	case DP_SHOW_SELECTED_MESH : 
		return DecorateBasePlugin::PerDocument;
...
void DecorateBasePlugin::DisplayCamera(QString who, Shotm &ls, int cameraSourceId)
{
	...
	if(ls.Intrinsics.cameraType == Camera<float>::PERSPECTIVE)
		typeBuf="Persp";
	if(ls.Intrinsics.cameraType == Camera<float>::ORTHO)     
		typeBuf="Ortho";
