...
QString FilterDocSampling::filterName(FilterIDType filterId) const
{
	switch(filterId) 
	{
		case FP_ELEMENT_SUBSAMPLING    :  
			return QString("Mesh Element Subsampling");
		case FP_MONTECARLO_SAMPLING :  
			return QString("Montecarlo Sampling");
		case FP_STRATIFIED_SAMPLING :  
			return QTextCodec::codecForName("GBK")->toUnicode("三角分层抽样") /*QString("Stratified Triangle Sampling")*/;
		case FP_CLUSTERED_SAMPLING :  
			return QTextCodec::codecForName("GBK")->toUnicode("聚类顶点二次抽样") /*QString("Clustered Vertex Subsampling")*/;
		case FP_POISSONDISK_SAMPLING : 
			return QString("Poisson-disk Sampling");
		case FP_HAUSDORFF_DISTANCE  :  
			return QString("Hausdorff Distance");
		case FP_DISTANCE_REFERENCE:  
			return QString("Distance from Reference Mesh");
		case FP_TEXEL_SAMPLING  :  
			return QString("Texel Sampling");
		case FP_VERTEX_RESAMPLING  : 
			return QString("Vertex Attribute Transfer");
		case FP_UNIFORM_MESH_RESAMPLING  : 
			return QString("Uniform Mesh Resampling");
		case FP_VORONOI_COLORING  : 
			return QString("Voronoi Vertex Coloring");
		case FP_DISK_COLORING  :  
			return QString("Disk Vertex Coloring");
		case FP_REGULAR_RECURSIVE_SAMPLING  :
			return QString("Regular Recursive Sampling");
		case FP_POINTCLOUD_SIMPLIFICATION  :
			return QString("Point Cloud Simplification");
...
QString FilterDocSampling::filterInfo(FilterIDType filterId) const
{
	...
	{
		case FP_ELEMENT_SUBSAMPLING  : 
			return QString("Create a new layer populated with a point sampling of the current mesh, At most one sample for each element of the mesh is created. Samples are taking in a uniform way, one for each element (vertex/edge/face); all the elements have the same probabilty of being choosen.");
		case FP_MONTECARLO_SAMPLING  : 
			return QString("Create a new layer populated with a point sampling of the current mesh; samples are generated in a randomly uniform way, or with a distribution biased by the per-vertex quality values of the mesh.");
		case FP_STRATIFIED_SAMPLING  :
			return QString(""); /*QString("Create a new layer populated with a point sampling of the current mesh; to generate multiple samples inside a triangle each triangle is subdivided according to various <i> stratified</i> strategies. Distribution is often biased by triangle shape.");*/
		case FP_CLUSTERED_SAMPLING   : 
			return QString(""); /*QString("Create a new layer populated with a  subsampling of the vertexes of the current mesh; the subsampling is driven by a simple one-per-gridded cell strategy.");*/
		case FP_POINTCLOUD_SIMPLIFICATION: 
			return QString("Create a new layer populated with a simplified version of the current point cloud.");
		case FP_POISSONDISK_SAMPLING : 
			return QString("Create a new layer populated with a point sampling of the current mesh;"
		...
		case FP_HAUSDORFF_DISTANCE   :
			return QString("Compute the Hausdorff Distance between two meshes, sampling one of the two and finding foreach sample the closest point over the other mesh.");
		case FP_DISTANCE_REFERENCE   : 
			return QString("Compute the signed/unsigned (per vertex) distance between a mesh/pointcloud and a reference mesh/pointcloud. Distance is stored in vertex quality.");
		case FP_TEXEL_SAMPLING       : 
			return QString("Create a new layer with a point sampling of the current mesh, a sample for each texel of the mesh is generated");
		case FP_VERTEX_RESAMPLING    :  
			return QString("Transfer the choosen per-vertex attributes from one mesh to another. Useful to transfer attributes to different representations of a same object.<br>"
		...
		case FP_UNIFORM_MESH_RESAMPLING       : 
			return QString("Create a new mesh that is a resampled version of the current one.<br>"
		...
		case FP_VORONOI_COLORING   : 
			return QString("Given a Mesh <b>M</b> and a Pointset <b>P</b>, The filter project each vertex of P over M and color M according to the geodesic distance from these projected points. Projection and coloring are done on a per vertex basis.");
		case FP_DISK_COLORING   : 
			return QString("Given a Mesh <b>M</b> and a Pointset <b>P</b>, The filter project each vertex of P over M and color M according to the Euclidean distance from these projected points. Projection and coloring are done on a per vertex basis.");
		case FP_REGULAR_RECURSIVE_SAMPLING   : 
			return QString("The bbox is recrusively partitioned in a octree style, center of bbox are considered, when the center is nearer to the surface than a given thr it is projected on it. It works also for building ofsetted samples.");
		default : 
			assert(0); 
			return QString("unknown filter!!!!");
	}
...
int FilterDocSampling::getRequirements(QAction *action)
{
	switch(ID(action))
	{
		case FP_DISK_COLORING :
		case FP_VORONOI_COLORING :
			return  MeshModel::MM_VERTFACETOPO  | MeshModel::MM_VERTQUALITY| MeshModel::MM_VERTCOLOR;
		case FP_VERTEX_RESAMPLING :
		case FP_UNIFORM_MESH_RESAMPLING:
		case FP_REGULAR_RECURSIVE_SAMPLING:
		case FP_HAUSDORFF_DISTANCE :	
			return  MeshModel::MM_FACEMARK;
		case FP_DISTANCE_REFERENCE :
		case FP_ELEMENT_SUBSAMPLING :
		case FP_MONTECARLO_SAMPLING :
		case FP_POISSONDISK_SAMPLING :
		...
		case FP_TEXEL_SAMPLING  :
			return MeshModel::MM_VERTCOLOR | MeshModel::MM_VERTNORMAL;
	...
	switch(ID(action))	
	{
		...
		case FP_STRATIFIED_SAMPLING :
		{
			parlst.addParam(new RichInt("SampleNum",  std::max(100000, md.mm()->cm.vn),
                                 QTextCodec::codecForName("GBK")->toUnicode("采样数:"), "" /*"Number of samples",
                                 "The desired number of samples. It can be smaller or larger than the mesh size, and according to the choosed sampling strategy it will try to adapt."*/));
			parlst.addParam(new RichEnum("Sampling", 0,
                                 QStringList() << QTextCodec::codecForName("GBK")->toUnicode("相似三角形") /*"Similar Triangle"*/ 
								 << QTextCodec::codecForName("GBK")->toUnicode("双重相似三角形") /*"Dual Similar Triangle"*/ 
								 << QTextCodec::codecForName("GBK")->toUnicode("长边细分") /*"Long Edge Subdiv"*/ 
								 << QTextCodec::codecForName("GBK")->toUnicode("边缘采样") /*"Sample Edges"*/ 
								 << QTextCodec::codecForName("GBK")->toUnicode("真实边缘采样") /*"Sample NonFaux Edges"*/,
                                 QTextCodec::codecForName("GBK")->toUnicode("采样元素:"), "" /*tr("Element to sample:"),
			...
			parlst.addParam(new RichBool("Random", false,
                                 QTextCodec::codecForName("GBK")->toUnicode("随机采样"), "" /*"Random Sampling",
		...
		case FP_CLUSTERED_SAMPLING :
		{
			...
			parlst.addParam(new RichAbsPerc("Threshold", maxVal*0.01,0,maxVal, QTextCodec::codecForName("GBK")->toUnicode("单元大小:"), "" /*"Cell Size", "The size of the cell of the clustering grid. Smaller the cell finer the resulting mesh. For obtaining a very coarse mesh use larger values."*/));

			parlst.addParam(new RichEnum("Sampling", 1,
                                 QStringList() << QTextCodec::codecForName("GBK")->toUnicode("平均值") /*"Average"*/ 
								 << QTextCodec::codecForName("GBK")->toUnicode("中心边缘") /*"Closest to center"*/,
                                 QTextCodec::codecForName("GBK")->toUnicode("典型策略"), "" /*tr("Representative Strategy:"),
			...
			parlst.addParam(new RichBool ("Selected", false, QTextCodec::codecForName("GBK")->toUnicode("已选中部分"), "" /*"Selected",
		...
		default: 
			break; // do not add any parameter for the other filters
	}
}
...
bool FilterDocSampling::applyFilter(QAction *action, MeshDocument &md, RichParameterSet & par, vcg::CallBackPos *cb)
{
	switch(ID(action))
	{
		case FP_ELEMENT_SUBSAMPLING :
		{
			...
			{
				case 0 :	
					tri::SurfaceSampling<CMeshO,BaseSampler>::VertexUniform(curMM->cm,mps,par.getInt("SampleNum"));
					break;
				case 1 :	
					tri::SurfaceSampling<CMeshO,BaseSampler>::EdgeUniform(curMM->cm,mps,par.getInt("SampleNum"),true);	
					break;
				case 2 :	
					tri::SurfaceSampling<CMeshO,BaseSampler>::AllFace(curMM->cm,mps);	
					break;
			}
		...
		case FP_STRATIFIED_SAMPLING :
		{
			if (md.mm()->cm.fn == 0) 
			{
				errorMessage = QTextCodec::codecForName("GBK")->toUnicode("这个过滤需要存在一个网格和一些面，不能仅仅是一些点集。") /*"This filter requires a mesh with some faces,<br> it does not work on PointSet"*/;
				return false; // can't continue, mesh can't be processed
			}
		...
		case FP_CLUSTERED_SAMPLING :
		{
			...
			switch(samplingMethod)
			{
				case 0 :
				{
					...
					Log("平均值采样创建了一个新的网格: %i个点", md.mm()->cm.vn);
				}
				break;
				case 1 :
				{
					...
					Log("中心边缘采样创建了一个新的网格: %i个点", md.mm()->cm.vn);
				}
				break;
			}
			vcg::tri::UpdateBounding<CMeshO>::Box(mm->cm);
		...
		case FP_VORONOI_COLORING :
		{
			...
			for(CMeshO::VertexIterator vi= mmV->cm.vert.begin(); vi!= mmV->cm.vert.end(); ++vi)
				if(!(*vi).IsD())
...
MeshFilterInterface::FilterClass FilterDocSampling::getClass(QAction *action)
{
	switch(ID(action))
	{
		...
		case FP_REGULAR_RECURSIVE_SAMPLING :
		case FP_TEXEL_SAMPLING  : 
			return FilterDocSampling::Sampling;
		case FP_UNIFORM_MESH_RESAMPLING:
			return FilterDocSampling::Remeshing;
		case FP_DISK_COLORING:
		case FP_VORONOI_COLORING:
			return MeshFilterInterface::FilterClass(FilterDocSampling::Sampling | FilterDocSampling::VertexColoring);
		case FP_POINTCLOUD_SIMPLIFICATION : 
			return MeshFilterInterface::FilterClass(FilterDocSampling::Sampling | FilterDocSampling::PointSet);
		default: 
			assert(0);
	}
