...
QString FilterLayerPlugin::filterName(FilterIDType filterId) const
{
    switch(filterId) 
	{
		...
		case FP_DUPLICATE :  
			return QTextCodec::codecForName("GBK")->toUnicode("复制当前图层") /*QString("Duplicate Current layer")*/;
		case FP_DELETE_MESH :  
			return QTextCodec::codecForName("GBK")->toUnicode("删除当前图层") /*return QString("Delete Current Mesh")*/;
		case FP_DELETE_NON_VISIBLE_MESH : 
			return QTextCodec::codecForName("GBK")->toUnicode("删除所有不可见图层") /*return QString("Delete all non visible Mesh Layers")*/;
		...
		case FP_FLATTEN :  
			return QTextCodec::codecForName("GBK")->toUnicode("合并可见图层") /*QString("Flatten Visible Layers")*/;
		case FP_RENAME_MESH :  
...
void FilterLayerPlugin::initParameterSet(QAction *action, MeshDocument &md, RichParameterSet & parlst)
{
    ...
    switch(ID(action))
    {
		...
		case FP_FLATTEN :
			{
				parlst.addParam(new RichBool ("MergeVisible", true, QTextCodec::codecForName("GBK")->toUnicode("合并可见图层"), ""  
				...
				parlst.addParam(new RichBool ("DeleteLayer", true, QTextCodec::codecForName("GBK")->toUnicode("删除原图层"), "" 
					/*"Delete Layers ",
				...
				//parlst.addParam(new RichBool ("MergeVertices", true,   QTextCodec::codecForName("GBK")->toUnicode("合并重复顶点"), "" 
				//	/*"Merge duplicate vertices",
				//	"Merge the vertices that are duplicated among different layers. \n\n"
				//	"Very useful when the layers are spliced portions of a single big mesh."*/));
				//parlst.addParam(new RichBool ("AlsoUnreferenced", false, QTextCodec::codecForName("GBK")->toUnicode("保持未引用顶点"), "" 
		...
		case FP_DUPLICATE :
			{
				...
				Log("已复制当前模型到第%i层", md.meshList.size());
		...
		case FP_FLATTEN :
			{
				...
				MeshModel *destMesh = md.addNewMesh("", QTextCodec::codecForName("GBK")->toUnicode("已合并网格") /*"Merged Mesh"*/);
				...
				{ 
					...
					if(mmp->visible || !mergeVisible)
					{
						if(mmp!= destMesh)
						{
							cb(cnt*100/md.meshList.size(), qPrintable(QTextCodec::codecForName("GBK")->toUnicode("正在合并图层...")));
				...
				if( deleteLayer )	
				{
					Log( "已删除%d个合并的层", toBeDeletedList.size());
				...
				if( mergeVertices )
				{
					...
					Log( "已删除%d个重复的顶点", delvert);
				}

				destMesh->UpdateBoxAndNormals();
				Log("已合并所有层到一个网格中: %i顶点", md.mm()->cm.vn);
