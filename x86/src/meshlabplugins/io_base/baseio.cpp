...
bool BaseMeshIOPlugin::open(const QString &formatName, const QString &fileName, MeshModel &m, int& mask, const RichParameterSet &parlst, CallBackPos *cb, QWidget * /*parent*/)
{
    ...
    if (cb != NULL)		
		(*cb)(0, qPrintable(QTextCodec::codecForName("GBK")->toUnicode("正在导入...")) /*"Loading..."*/);

    QString errorMsgFormat = QTextCodec::codecForName("GBK")->toUnicode("出现错误在导入文件") + "%1:\n%2"; /*"Error encountered while loading file:\n\"%1\"\n\nError details: %2";*/
    ...
    if (someTextureNotFound)
        Log("丢失的纹理文件: %s" /*"Missing texture files: %s"*/, qPrintable(missingTextureFilesMsg));

    if (cb != NULL)	
		(*cb)(99, qPrintable(QTextCodec::codecForName("GBK")->toUnicode("已成功完成")) /*"Done"*/);
...
{
    QString errorMsgFormat = QTextCodec::codecForName("GBK")->toUnicode("出现错误在导出文件") + "%1:\n%2"; /*"Error encountered while exportering file %1:\n%2";*/
...
{
    if(format.toUpper() == tr("STL"))
        par.addParam(new RichBool("Unify", true, QTextCodec::codecForName("GBK")->toUnicode("合并重复顶点"), "" 
...
void BaseMeshIOPlugin::initSaveParameter(const QString &format, MeshModel &/*m*/, RichParameterSet &par)
{
    if(format.toUpper() == tr("STL") || format.toUpper() == tr("PLY"))
        par.addParam(new RichBool("Binary", true, QTextCodec::codecForName("GBK")->toUnicode("二进制"), ""
		                        /* "Binary encoding",
                                "Save the mesh using a binary encoding. If false the mesh is saved in a plain, readable ascii format"*/));

    if(format.toUpper() == tr("STL") )
		par.addParam(new RichBool("ColorMode", true, QTextCodec::codecForName("GBK")->toUnicode("颜色二进制编码"), "" 
