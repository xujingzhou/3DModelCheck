...
MeshlabStdDialog::MeshlabStdDialog(QWidget *p)
	:QDockWidget(QTextCodec::codecForName("GBK")->toUnicode("插件") /*QString("Plugin")*/, p), previewCB(NULL), curmask(MeshModel::MM_UNKNOWN)
{
	qf = NULL;
	stdParFrame = NULL;
	clearValues();
...
bool MeshlabStdDialog::showAutoDialog(MeshFilterInterface *mfi, MeshModel *mm, MeshDocument * mdp, QAction *action, MainWindow *mwi, QWidget *gla)
{
	...
	if (curParSet.isEmpty() && !isPreviewable()) 
		return false;
...
//	QPushButton *helpButton = new QPushButton(QTextCodec::codecForName("GBK")->toUnicode("帮助") /*"Help"*/, qf);
//    QPushButton *closeButton = new QPushButton(QTextCodec::codecForName("GBK")->toUnicode("关闭") /*"Close"*/, qf);
    QPushButton *applyButton = new QPushButton(QTextCodec::codecForName("GBK")->toUnicode("应用") /*"Apply"*/, qf);
    QPushButton *defaultButton = new QPushButton(QTextCodec::codecForName("GBK")->toUnicode("默认") /*"Default"*/, qf);
	...
	if (isPreviewable())
	{
		previewCB = new QCheckBox(QTextCodec::codecForName("GBK")->toUnicode("预览"), qf);
	...
	gridLayout->addWidget(applyButton, buttonRow + 0, 1, Qt::AlignBottom);
