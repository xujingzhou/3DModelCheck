...
MeshLabXMLStdDialog::MeshLabXMLStdDialog(QWidget *p)
    :QDockWidget(QTextCodec::codecForName("GBK")->toUnicode("插件") /*QString("Plugin")*/, p),isfilterexecuting(false),env(),showHelp(false),previewCB(NULL)
{
    curmask = 0;
    qf = NULL;
    stdParFrame=NULL;
...
//	helpButton = new QPushButton(QTextCodec::codecForName("GBK")->toUnicode("帮助") /*"Help"*/, qf);
    //helpButton->setSizePolicy(QSizePolicy::Preferred,QSizePolicy::Minimum);
//    closeButton = new QPushButton(QTextCodec::codecForName("GBK")->toUnicode("关闭") /*"Close"*/, qf);
    //closeButton->setSizePolicy(QSizePolicy::Preferred,QSizePolicy::Minimum);
    applyButton = new QPushButton(QTextCodec::codecForName("GBK")->toUnicode("应用") /*"Apply"*/, qf);
    //applyButton->setSizePolicy(QSizePolicy::Preferred,QSizePolicy::Minimum);
    defaultButton = new QPushButton(QTextCodec::codecForName("GBK")->toUnicode("默认") /*"Default"*/, qf);
    ...
    if(isPreviewable())
    {
        previewCB = new QCheckBox(QTextCodec::codecForName("GBK")->toUnicode("预览") /*"Preview"*/, qf);
...
bool MeshLabXMLStdDialog::showAutoDialog(MeshLabXMLFilterContainer& mfc,PluginManager& pm,MeshDocument * md, MainWindowInterface *mwi, QWidget *gla/*=0*/ )
{
    ...
    curMeshDoc->Log.Logf(GLLogStream::SYSTEM, "界面创建花费了: %i秒", tt.elapsed());
...
{
    if (applystate)
        return QTextCodec::codecForName("GBK")->toUnicode("应用") /*QString("Apply")*/;
    else
        return QTextCodec::codecForName("GBK")->toUnicode("停止") /*QString("Stop")*/;
    return QString();
}
    ...
    :XMLMeshLabWidget(xmlWidgetTag,envir,parent)
{
    ...
    fieldDesc = new QLabel(xmlWidgetTag[MLXMLElNames::guiLabel] + QTextCodec::codecForName("GBK")->toUnicode(" (绝对值和百分比)") /*" (abs and %)"*/,this);
    fieldDesc->setToolTip(xmlWidgetTag[MLXMLElNames::paramHelpTag]);
    ...
    absLab = new QLabel("<i> <small>" + QTextCodec::codecForName("GBK")->toUnicode("世界坐标")/* world unit*/ + "</small></i>", this);
    percLab = new QLabel("<i> <small>" + QTextCodec::codecForName("GBK")->toUnicode("perc在")/* perc on"*/ + QString("(%1 .. %2)").arg(m_min).arg(m_max) + "</small></i>", this);
    ...
    :XMLMeshLabWidget(xmlWidgetTag,envir,p)
{
    ...
    if (par != NULL)
    {
        ...
        {
            getPoint3Button = new QPushButton(QTextCodec::codecForName("GBK")->toUnicode("获取") /*"Get"*/, this);
            ...
            QStringList names;
			names << QTextCodec::codecForName("GBK")->toUnicode("视图目录"); //"View Dir";
			names << QTextCodec::codecForName("GBK")->toUnicode("视图位置"); //"View Pos";
			names << QTextCodec::codecForName("GBK")->toUnicode("表面位置"); //"Surf. Pos";
			names << QTextCodec::codecForName("GBK")->toUnicode("相机位置"); //"Camera Pos";
			names << QTextCodec::codecForName("GBK")->toUnicode("跟踪球中心"); //"Trackball Center";
    ...
    :XMLMeshLabWidget(xmlWidgetTag,envir,p)
{
    ...
    //gridLay->addWidget(descLab,row,0,Qt::AlignTop);
    hlay = new QHBoxLayout();
    getShotButton = new QPushButton(QTextCodec::codecForName("GBK")->toUnicode("获取截图") /*"Get shot"*/,this);
    ...
    {
		names << QTextCodec::codecForName("GBK")->toUnicode("当前跟踪球"); //"Current Trackball";
        names << QTextCodec::codecForName("GBK")->toUnicode("当前网格"); //"Current Mesh";
        names << QTextCodec::codecForName("GBK")->toUnicode("当前栅图"); //"Current Raster";
    ...
    names << QTextCodec::codecForName("GBK")->toUnicode("来自文件"); //"From File";
...
void XMLShotWidget::getShot()
{
    ...
    case 3:
        {
            QString filename = QFileDialog::getOpenFileName(this, QTextCodec::codecForName("GBK")->toUnicode("加载xml相机") /*tr("Load xml camera")*/, "./", tr("Xml Files (*.xml)"));
            QFile qf(filename);
...
void OldScriptingSystemXMLParamDialog::createFrame()
{
    if ((_mfc.act != NULL) && (_mfc.xmlInfo != NULL))
    {
		...
		buttonBox->button(QDialogButtonBox::Reset)->setText(QTextCodec::codecForName("GBK")->toUnicode("复位"));
		buttonBox->button(QDialogButtonBox::Help)->setText(QTextCodec::codecForName("GBK")->toUnicode("帮助"));
