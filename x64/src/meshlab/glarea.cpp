    ...
    : QGLWidget(parent,mvcont->sharedDataContext()),interrbutshow(false)
{
    ...
    */
	...
	// Zoom in/Zoom out multiple will be limited. Added by Johnny Xu, 2017/5/2
	zoomInLimit = 8;
	zoomOutLimit = 5;
	zoomInFlag = 1.0f;
	zoomOutFlag = -1.0f;
    ...
    if (tileCol >= totalCols)
    {
        ...
        if (tileRow >= totalRows)
        {
            ...
            if(!ss.tiledSave)
            {
                ...
                if (ret)
                {
                    this->Logf(GLLogStream::SYSTEM, "截图被保存到: %s", outfile.toLocal8Bit().constData());
                    if(ss.addToRasters)
                    {
                ...
                {
                    Logf(GLLogStream::WARNING, "保存错误: %s", outfile.toLocal8Bit().constData());
                }
            }
...
void GLArea::paintEvent(QPaintEvent* /*event*/)
{
    ...
    if(hasToPick && !hasToGetPickPos)
    {
        ...
        if(Pick<Point3f>(pointToPick[0],pointToPick[1],pp))
        {
            ...
            Logf(0, "确定中心点: [%f %f %f] [%d,%d]", pp[0], pp[1], pp[2], pointToPick[0], pointToPick[1]);
        }
    }
    ...
    QString error = checkGLError::makeString("这儿存在着OpenGL错误 ");
    if(!error.isEmpty()) 
	{
	...
	if (infoAreaVisible)
	{
		...
		// Fix bug #19 by Johnny Xu,2017/2/15
//		displayInfo(&painter);
...
void GLArea::displayInfo(QPainter *painter)
{
    ...
    if(this->md()->size()>0)
    {
        if(this->md()->size() == 1)
        {
			col1Text += QTextCodec::codecForName("GBK")->toUnicode("网格: ") + QString("%1\n").arg(mm()->label());
            col1Text += QTextCodec::codecForName("GBK")->toUnicode("顶点: ") + QString("%1\n").arg(mm()->cm.vn);
            col1Text += QTextCodec::codecForName("GBK")->toUnicode("面: ") + QString("%1\n").arg(mm()->cm.fn);
        }
        else
        {
			QFileInfo inf = mm()->label();
			col1Text += QTextCodec::codecForName("GBK")->toUnicode("当前网格: ") + QString("%1\n").arg(inf.completeBaseName());
            col1Text += QTextCodec::codecForName("GBK")->toUnicode("顶点: ") + QString("%1 (%2)\n").arg(mm()->cm.vn).arg(this->md()->vn());
            col1Text += QTextCodec::codecForName("GBK")->toUnicode("面: ") + QString("%1 (%2)\n").arg(mm()->cm.fn).arg(this->md()->fn());
        }
		...
		col1Text += QTextCodec::codecForName("GBK")->toUnicode("选中 - 顶点:") + QString("%1").arg(svn) + QTextCodec::codecForName("GBK")->toUnicode(" 面:") + QString("%1\n").arg(sfn);
                
        col1Text += GetMeshInfoString();

        if(fov>5) 
			col0Text += QTextCodec::codecForName("GBK")->toUnicode("视场角: ") + QString("%1\n").arg(fov);
        else 
			col0Text += QTextCodec::codecForName("GBK")->toUnicode("视场角: 正交模式") /* QString("FOV: Ortho\n")*/;

        if ((cfps>0) && (cfps<1999))
            col0Text += QTextCodec::codecForName("GBK")->toUnicode("每秒帧数:") + QString("%1\n").arg(cfps,7,'f',1);

		col0Text += renderfacility;

        if (clipRatioNear!=clipRatioNearDefault())
            col0Text += QTextCodec::codecForName("GBK")->toUnicode("近剪切:") + QString("%1\n").arg(clipRatioNear,7,'f',2);
        ...
        if(mm()->cm.Tr != Matrix44m::Identity() ) 
			displayMatrix(painter, Column_2);
    }
...
void GLArea::displayHelp()
{
    ...
    if(tableText.isEmpty())
    {
        ...
        if(helpFile.open(QFile::ReadOnly))
            // Modified by Johnny Xu, 2017/2/3
            tableText = QTextCodec::codecForName("GBK")->toUnicode(helpFile.readAll());
        else assert(0);
#ifdef Q_OS_MAC
    ...
    md()->Log.RealTimeLog(QTextCodec::codecForName("GBK")->toUnicode("快速帮助"),"",tableText);
}
...
void GLArea::setCurrentEditAction(QAction *editAction)
{
	...
	else
	{
		...
		{
			QMessageBox *box = new QMessageBox(QMessageBox::Warning, QString::fromLocal8Bit("提示"), QString::fromLocal8Bit("拼接至少需要导入两个模型!"));
    ...
    {
		// Modified by Johnny Xu, 2017/2/20
		Logf(GLLogStream::SYSTEM, "已启用模式: %s", qPrintable(currentEditor->text()));
...
void GLArea::mousePressEvent(QMouseEvent*e)
{
	...
	{
		// Zoom in/Zoom out multiple will be limited. Added by Johnny Xu, 2017/5/3
		lastPosX = e->x();
		lastPosY = e->y();
...
void GLArea::mouseMoveEvent(QMouseEvent*e)
{
    ...
    else 
	{
        ...
        {
			// Zoom in/Zoom out multiple will be limited. Added by Johnny Xu, 2017/5/3
			qDebug("e->x: %d; e->y: %d", e->x(), e->y());
			if (e->modifiers() & Qt::ShiftModifier)
			{
				if (this->geometry().contains(e->pos()))
				{
					qDebug("Mounse position in the area");
				...
				{
					qDebug("Mounse position out of area");
				}
			}
			else
			{
    ...
    else
    {
        ...
        default:
            {            
				...
				{
					// Zoom in/Zoom out multiple will be limited. Added by Johnny Xu, 2017/5/2
					qDebug("notch: %f", notch);
					if (qAbs(notch - zoomInFlag) <= epsilon)
					{
						// Zoom in(1.0f)
						qDebug("Zoom in. - zoomInStack.size(): %d; zoomOutStack.size(): %d", zoomInStack.size(), zoomOutStack.size());
						...
						{
							if (zoomInStack.size() == zoomInLimit)
							{
								notch = 0.f;
								qDebug("Zoom in multiple is full. zoomInLimit: %d", zoomInLimit);
							}
							else
							{
								zoomInStack.push(zoomInFlag);
							}
						}
					}
					else if (qAbs(notch - zoomOutFlag) <= epsilon)
					{
						// Zoom out(-1.0f)
						qDebug("Zoom out. - zoomOutStack.size(): %d; zoomInStack.size(): %d", zoomOutStack.size(), zoomInStack.size());
						...
						{
							if (zoomOutStack.size() == zoomOutLimit)
							{
								notch = 0.f;
								qDebug("Zoom out multiple is full. zoomOutLimit: %d", zoomOutLimit);
							}
							else
							{
								zoomOutStack.push(zoomOutFlag);
							}
						}
					}
					else
					{
						notch = 0.f;
						qDebug("Exception - notch = 0: %f", notch);
					}
...
void GLArea::setCursorTrack(vcg::TrackMode *tm)
{
    ...
    if(tm) 
		setCursor(curMap[tm->Name()]);
    else
        if(hasToGetPickPos) 
			setCursor(curMap["PickMode"]);
        else 
			setCursor(curMap[""]);
    ...
    if(iDecorateTemp->getDecorationClass(action)== MeshDecorateInterface::PerDocument)
    {
        ...
        if(found)
        {
            if(toggle || stateToSet==false)
			{
                ...
                this->Logf(GLLogStream::SYSTEM, "已禁止装饰模式: %s", qPrintable(action->text()));
            } 
			else
                this->Logf(GLLogStream::SYSTEM, "尝试禁止一个已禁止的装饰模式: %s", qPrintable(action->text()));
        }
        else
		{
            if(toggle || stateToSet==true)
			{
                ...
                if(ret) 
				{
                    ...
                    this->Logf(GLLogStream::SYSTEM, "打开装饰模式: %s", qPrintable(action->text()));
                }
                else 
					this->Logf(GLLogStream::SYSTEM, "启动装饰模式已失败: %s", qPrintable(action->text()));
            } 
			else
                this->Logf(GLLogStream::SYSTEM, "尝试打开一个已打开的装饰模式: %s", qPrintable(action->text()));
        }
    }
    ...
    if(iDecorateTemp->getDecorationClass(action)== MeshDecorateInterface::PerMesh)
    {
        ...
        if(found)
        {
            if(toggle || stateToSet == false)
			{
                ...
                iDecorateTemp->setLog(NULL);
                this->Logf(0, "禁止装饰模式: %s", qPrintable(action->text()));
            } 
			else
                this->Logf(GLLogStream::SYSTEM, "尝试禁止一个已禁止的装饰模式: %s", qPrintable(action->text()));
        }
        else
		{
            if(toggle || stateToSet == true)
			{
                ...
                if (iDecorateTemp->isDecorationApplicable(action,currentMeshModel,errorMessage)) 
                {
                    ...
                    else this->Logf(GLLogStream::SYSTEM, "失败的装饰模式: %s", qPrintable(action->text()));
                } 
				else
                    this->Logf(GLLogStream::SYSTEM, "错误的装饰模式%s: %s", qPrintable(action->text()),qPrintable(errorMessage));
...
void GLArea::loadRaster(int id)
{
    ...
    foreach(RasterModel *rm, this->md()->rasterList)
        if(rm->id()==id)
        {
            ...
            {
                Logf(0, "图片文件: %s没有被正确载入！", rm->currentPlane->fullPathFileName.toUtf8().constData());
...
{
    shot.Intrinsics.PixelSizeMm[0] = (float)0.036916077;
    shot.Intrinsics.PixelSizeMm[1] = (float)0.036916077;
...
{
    Shotm localShot;
    if(kind=="Mesh" && this->md()->mm())   
		localShot = this->md()->mm()->cm.shot;
    if(kind=="Raster" && this->md()->rm()) 
		localShot = this->md()->rm()->shot;

    if(!localShot.IsValid())
    {
        this->Logf(GLLogStream::SYSTEM, "不能设置快照来自: %s", qPrintable(kind));
        return;
    }
...
void GLArea::createOrthoView(QString dir)
{
	...
	this->Logf(GLLogStream::SYSTEM, "视图场景来自: %s", qPrintable(dir));
}
