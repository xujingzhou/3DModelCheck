...
{   
    ...
    _shadelab = new QLabel(QTextCodec::codecForName("GBK")->toUnicode("着色") /*"Shading"*/, this);
    QFont boldfont;
    ...
    _colorlab = new QLabel( QTextCodec::codecForName("GBK")->toUnicode("颜色") /*"Color"*/, this);
	...
	_backfacelab = new QLabel( QTextCodec::codecForName("GBK")->toUnicode("背面") /* "Back-Face"*/, this);
	...
	_textlab = new QLabel( QTextCodec::codecForName("GBK")->toUnicode("纹理坐标") /*"Texture Coord"*/, this);
 ...
 */   
    ...
    QLabel* dimelab = new QLabel(QTextCodec::codecForName("GBK")->toUnicode("边缘宽度") /*"Edge Width"*/, this);
    ...
    _wirelab = new QLabel(QTextCodec::codecForName("GBK")->toUnicode("多边形形态") /*"Polygonal Modality"*/, this);
...
void MLRenderingPointsParametersFrame::initGui()
{
    ...
    QLabel* dimelab = new QLabel(QTextCodec::codecForName("GBK")->toUnicode("点大小") /*"Point Size"*/, this);
    dimelab->setFont(boldfont);
...
void MLRenderingBBoxParametersFrame::initGui()
{
    ...
    _quotedinfolab = new QLabel(QTextCodec::codecForName("GBK")->toUnicode("测量信息") /*"Measure Info"*/, this);
...
void MLRenderingDefaultDecoratorParametersFrame::initGui()
{
    ...
    QLabel* boundarylab = new QLabel(QTextCodec::codecForName("GBK")->toUnicode("边界边缘") /*"Boundary Edges"*/, this);
    QFont boldfont;
    boldfont.setBold(true);
    ...
    QLabel* boundaryfacelab = new QLabel(QTextCodec::codecForName("GBK")->toUnicode("边界面") /*"Boundary Faces"*/, this);
    ...
    QLabel* vertmanifoldlab = new QLabel(QTextCodec::codecForName("GBK")->toUnicode("非流型顶点") /*"No-Manif Verts"*/, this);
    ...
    QLabel* edgemanifoldlab = new QLabel(QTextCodec::codecForName("GBK")->toUnicode("非流型边缘") /*"No-Manif Edges"*/, this);
    ...
    QLabel* textureborderlab = new QLabel(QTextCodec::codecForName("GBK")->toUnicode("纹理边界") /*"Texture Border"*/, this);
...
void MLRenderingSelectionParametersFrame::initGui()
{
    ...
    QLabel* vertlab = new QLabel(QTextCodec::codecForName("GBK")->toUnicode("选中的顶点") /*"Selected Vertex"*/, this);
    QFont boldfont;
    boldfont.setBold(true);
    ...
    QLabel* facelab = new QLabel(QTextCodec::codecForName("GBK")->toUnicode("选中的面") /*"Selected Face"*/, this);
...
void MLRenderingColorPicker::updateColorInfo()
{
	...
	// Fix color can't show issue by Johnny Xu, 2017/7/10
	QString strColor = QString("#%1%2%3").arg(cc.red(), 2, 16, QChar('0'))
		.arg(cc.green(), 2, 16, QChar('0')).arg(cc.blue(), 2, 16, QChar('0'));
...
void MLRenderingColorPicker::pickColor()
{
	...
	// Fix color dialog by Johnny Xu, 2017/7/10
	QColorDialog *colorDialog = new QColorDialog();
	if (QDialogButtonBox *btnBox = colorDialog->findChild<QDialogButtonBox *>())
	{
		if (QPushButton *btnCancel = btnBox->button(QDialogButtonBox::Cancel))
			btnCancel->setText(QString::fromLocal8Bit("取消"));

		if (QPushButton *btnCancel = btnBox->button(QDialogButtonBox::Ok))
			btnCancel->setText(QString::fromLocal8Bit("确定"));

		//QObjectList btnList = colorDialog->children();
		//QPushButton *btn;
		//foreach(QObject *obj, btnList)
		//{
		//	btn = qobject_cast<QPushButton*>(obj);
		//	if (btn)
		//	{
		//		QString strBtn = btn->text();
		//		if (strBtn == QString("&Pick Screen Color"))
		//		{
		//			btn->setText(QString::fromLocal8Bit("&提取颜色"));
		//		}
		//	}
		//}
	}
	...
	QSkinObject *skin = new QSkinObject(colorDialog);
	skin->startSkinning();
	colorDialog->setSizeGripEnabled(false);
	// Added by Johnny Xu, 2017/7/19
	colorDialog->setWindowFlags(windowFlags()&~Qt::WindowMinMaxButtonsHint&~Qt::WindowContextHelpButtonHint);

	colorDialog->setCurrentColor(cc);
	int dialogRet = colorDialog->exec();
	QColor ret = colorDialog->selectedColor();
...
void MLRenderingOnOffToolbar::initGui()
{
    ...
    _onact = addAction( QTextCodec::codecForName("GBK")->toUnicode("开") /*QString("On")*/);
    _offact = addAction( QTextCodec::codecForName("GBK")->toUnicode("关") /*QString("Off")*/);
...
void MLRenderingOnOffToolbar::toggle(QAction* clickedact)
{
    if ((_act != NULL) && (clickedact != NULL))
    {
		...
		else
		{
			...
			emit updateRenderingDataAccordingToAction(_meshid, _act, (clickedact->text() ==  QTextCodec::codecForName("GBK")->toUnicode("开") /*QString("On")*/));
		}
    }
}
...
void MLRenderingGlobalToolbar::enable(bool isEnable /* = true*/)
{
	foreach(QAction* act, actions())
	{
		if (act != NULL)
...
// 	_solidactgroup->addAction(_flatglobact);
	...
	MLRenderingSelectionAction* selectAct = new MLRenderingSelectionAction(this);
	selectAct->setChecked(false);
	_selectglobact = new MLRenderingGlobalAction(selectAct->text(), selectAct->icon(), this);
	_selectglobact->addMainAction(selectAct);
	_selectglobact->setCheckable(true);
	addAction(_selectglobact);

	MLRenderingEdgeDecoratorAction* decoratorAction = new MLRenderingEdgeDecoratorAction(this);
	decoratorAction->setChecked(false);
	_decorateglobact = new MLRenderingGlobalAction(decoratorAction->text(), decoratorAction->icon(), this);
	_decorateglobact->addMainAction(decoratorAction);
	_decorateglobact->setCheckable(true);
	addAction(_decorateglobact);
