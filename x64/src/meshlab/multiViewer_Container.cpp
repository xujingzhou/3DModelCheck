...
void MultiViewer_Container::closeEvent( QCloseEvent *event )
{
	...
	{
		QMessageBox *box = new QMessageBox(QMessageBox::Warning, QTextCodec::codecForName("GBK")->toUnicode("提示"), QTextCodec::codecForName("GBK")->toUnicode("存在网格") + QString("'%1'").arg(meshDoc.getModifiedMeshLabel().join(",")) + QTextCodec::codecForName("GBK")->toUnicode("未被保存！\n\n确认要关闭么？"));
		box->setStandardButtons(QMessageBox::Yes | QMessageBox::No);
		box->setButtonText(QMessageBox::Yes, QTextCodec::codecForName("GBK")->toUnicode("是"));
		box->setButtonText(QMessageBox::No, QTextCodec::codecForName("GBK")->toUnicode("否"));
