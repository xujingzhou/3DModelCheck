	...
	int PreferredGridSize)
{
	...
	if (PreferredGridSize == 0)
		PreferredGridSize = fm->vert.size()*pp.UGExpansionFactor;
	...
	int PreferredGridSize)
{
	...
	//Inserisco la src nella griglia
	if (PreferredGridSize == 0)
		PreferredGridSize = fm->face.size()*pp.UGExpansionFactor;
	...
	AlignPair::Stat &as)
{
	...
	do
	{
			...
			if (beyondCntVec[i] < maxBeyondCnt)
			{
				...
				if (fixbox.IsIn(movvert[i]))
				{
					...
					if (u.Empty()) // using the point cloud grid
					{
						...
						{
							ii.DistanceDiscarded++; ++beyondCntVec[i]; 
							continue;
						}
		...
		case AlignPair::Param::MMSimilarity: 
			ComputeRotoTranslationScalingMatchMatrix(newout, Pfix, OPmov); 
			break;
		case AlignPair::Param::MMRigid: 
			ComputeRigidMatchMatrix(Pfix, OPmov, newout);   
			break;
		default:
			status = UNKNOWN_MODE;
			ii.Time = clock();
