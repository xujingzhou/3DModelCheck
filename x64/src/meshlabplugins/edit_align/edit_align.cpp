...
#include "meshlab/skin/qskinobject.h"
#include "meshlab/skin/titlebar_dialog.h"
...
{
    return QTextCodec::codecForName("GBK")->toUnicode("拼接不同的层到一起"); /*tr("Allow to align different layers together.")*/;
}
...
//    it.value().colorMode=GLW::CMPerMesh;
	...
	// Remove cursor by Johnny Xu, 2017/8/2
 /*   _gla->setCursor(QCursor(QPixmap(":/images/cur_align.png"),1,1));*/
    ...
    if(alignDialog == 0)
    {
		...
		skin->setWindowTitle(alignDialog->windowTitle());
		alignDialog->setTitleBarWidget(skin->getTitleBar());
//		alignDialog->setAttribute(Qt::WA_TranslucentBackground);;
#else  
		// Normal
#endif
   ...
   //     connect(alignDialog->ui.manualAlignButton,SIGNAL(clicked()),this,SLOT(glueManual()));
	...
	alignDialog->setFixedSize(alignDialog->size());
...
void EditAlignPlugin::glueByPicking()
{
	...
	{
		QMessageBox *box = new QMessageBox(QMessageBox::Warning, QString::fromLocal8Bit("拼接工具"), QString::fromLocal8Bit("基于点的拼接至少需要一个网格!"));
    ...
    dd->alignPairDialog->initMesh(currentNode(), &meshTree);

#if defined (_CUSTOM_SKIN_)  

	// Cusctom titlebar by Johnny Xu, 2017/7/13
	SkinDialog *skin = new SkinDialog(dd);
	skin->setWindowTitle(dd->windowTitle());
	skin->setWindowIcon(dd->windowIcon());
	// Set up to model dialog by Johnny Xu, 2017/7/18
	skin->setModal(true);
	skin->show();
	skin->move((QApplication::desktop()->width() - skin->width()) / 2, (QApplication::desktop()->height() - skin->height()) / 2);
    ...
    std::vector<vcg::Point3f> freePnt = dd->alignPairDialog->freePickedPointVec;
    std::vector<vcg::Point3f> gluedPnt = dd->alignPairDialog->gluedPickedPointVec;
    ...
    if( (freePnt.size() != gluedPnt.size())	|| (freePnt.size() == 0) )	
	{
		...
		QMessageBox *box = new QMessageBox(QMessageBox::Warning, QString::fromLocal8Bit("拼接工具"), QString::fromLocal8Bit("需要选择相同数量的点!"));
		box->setStandardButtons(QMessageBox::Ok);
		box->setButtonText(QMessageBox::Ok, QString::fromLocal8Bit("确定"));
		...
		// Cusctom titlebar by Johnny Xu, 2017/7/13
		QSkinObject *skinObject = new QSkinObject(box);
    ...
    Matrix44f res;
    if ((dd != NULL) && (dd->alignPairDialog != NULL) && (dd->alignPairDialog->allowscaling))
...
void EditAlignPlugin::glueManual()
{
	...
	//		alignDialog->ui.manualAlignButton->setText(QTextCodec::codecForName("GBK")->toUnicode("接受转换") /*"Accept Transformation"*/);
			break;
		}
	...
	//		alignDialog->ui.manualAlignButton->setText(oldLabelButton);
		...
		default : 
			assert("entered in the GlueManual slot in the wrong state"==0);
	}
...
void EditAlignPlugin:: alignParamCurrent()
{
    ...
    RichParameterSet alignParamSet;
     QString titleString = QTextCodec::codecForName("GBK")->toUnicode("当前弧度") + QString(" (%1 -> %2) ").arg(currentArc()->MovName).arg(currentArc()->FixName) + QTextCodec::codecForName("GBK")->toUnicode("拼接参数");
...
void EditAlignPlugin:: meshTreeParam()
{
  ...
  GenericParamDialog ad(alignDialog, &meshTreeParamSet, QTextCodec::codecForName("GBK")->toUnicode("默认拼接参数") /*"Default Alignment Parameters"*/);
  ad.setWindowFlags(Qt::Dialog);
  ad.setWindowModality(Qt::WindowModal);
  int result=ad.exec();
...
void EditAlignPlugin:: alignParam()
{
    ...
    GenericParamDialog ad(alignDialog, &alignParamSet, QTextCodec::codecForName("GBK")->toUnicode("默认拼接参数") /*"Default Alignment Parameters"*/);
    ad.setWindowFlags(Qt::Dialog);
    ad.setWindowModality(Qt::WindowModal);
    int result=ad.exec();
...
void EditAlignPlugin::setAlignParamMM()
{
	...
	QMessageBox *box = new QMessageBox(QMessageBox::Warning, QString::fromLocal8Bit("拼接工具"), QString::fromLocal8Bit("ICP默认参数设置成毫米!"));
	box->setStandardButtons(QMessageBox::Ok);
	box->setButtonText(QMessageBox::Ok, QString::fromLocal8Bit("确定"));
...
void EditAlignPlugin::setAlignParamM()
{
	...
	QMessageBox *box = new QMessageBox(QMessageBox::Warning, QString::fromLocal8Bit("拼接工具"), QString::fromLocal8Bit("ICP默认参数设置成米!"));
	box->setStandardButtons(QMessageBox::Ok);
	box->setButtonText(QMessageBox::Ok, QString::fromLocal8Bit("确定"));
...
void EditAlignPlugin::process()
{
    ...
    {
		QMessageBox *box = new QMessageBox(QMessageBox::Warning, QString::fromLocal8Bit("拼接工具"), QString::fromLocal8Bit("处理过程仅工作在至少2个网格被黏合的情况下!"));
		box->setStandardButtons(QMessageBox::Ok);
		box->setButtonText(QMessageBox::Ok, QString::fromLocal8Bit("确定"));
