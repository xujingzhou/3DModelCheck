	...
	if(formatName.toUpper() == tr("U3D"))
	{
	  ...
	  QFileInfo converterFI(converterPath);
		if(!converterFI.exists())
		{
			...
			QString strBox = QString::fromLocal8Bit("缺失可执行的转换器 '%1'").arg(converterPath);
		...
		if(result!=0)
		{
			QString strBox = errorMsgFormat.arg(fileName, vcg::tri::io::ExporterU3D<CMeshO>::ErrorMsg(result));
			QMessageBox box(QMessageBox::Warning, QString::fromLocal8Bit("保存错误"), strBox);
			box.setStandardButtons(QMessageBox::Ok);
			box.setButtonText(QMessageBox::Ok, QString::fromLocal8Bit("确 定"));
			box.exec();
