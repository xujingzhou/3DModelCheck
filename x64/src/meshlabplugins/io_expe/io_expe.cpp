...
bool ExpeIOPlugin::open(const QString &formatName, const QString &fileName, MeshModel &m, int& mask, const RichParameterSet &parlst, CallBackPos *cb, QWidget *parent)
{
    ...
    //std::cout << "loadMask = " << loadMask << "\n";
		...
		if (result != 0)
		{
			QString strBox = errorMsgFormat.arg(fileName, vcg::tri::io::ImporterExpePTS<CMeshO>::ErrorMsg(result));
			QMessageBox box(QMessageBox::Warning, QString::fromLocal8Bit("打开错误"), strBox);
  ...
  else if (formatName.toLower() == tr("xyz"))
  {
    ...
    if (result != 0)
	{
		QString strBox = errorMsgFormat.arg(fileName, vcg::tri::io::ImporterXYZ<CMeshO>::ErrorMsg(result));
		QMessageBox box(QMessageBox::Warning, QString::fromLocal8Bit("打开错误"), strBox);
		box.setStandardButtons(QMessageBox::Ok);
		box.setButtonText(QMessageBox::Ok, QString::fromLocal8Bit("确 定"));
		box.exec();
