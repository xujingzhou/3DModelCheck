...
ExtraMeshFilterPlugin::ExtraMeshFilterPlugin(void)
{
    ...
    foreach(tt, types())
	{
		...
		if(tt == FP_CLOSE_HOLES)
		{
            actionList.last()->setIcon(QIcon(":/images/hole_filler.png"));
			actionList.last()->setPriority(QAction::NormalPriority);
      }
	}
...
ExtraMeshFilterPlugin::FilterClass ExtraMeshFilterPlugin::getClass(QAction * a)
{
    switch (ID(a))
    {
		...
		// Moved 'FP_CLOSE_HOLES' from Remeshing to Cleaning. Modified by Johnny Xu, 2017/2/22
		case FP_CLOSE_HOLES                      :
		...
		case FP_QUAD_PAIRING                     : 
			return FilterClass(Remeshing+Polygonal);

		case FP_NORMAL_EXTRAPOLATION             : 
			return FilterClass( Normal + PointSet );
		...
		case FP_RESET_TRANSFORM                  : 
			return FilterClass(Normal + Layer);
...
QString ExtraMeshFilterPlugin::filterName(FilterIDType filter) const
{
    ...
    {
		case FP_LOOP_SS                          : 
			return tr("Subdivision Surfaces: Loop");
		case FP_BUTTERFLY_SS                     : 
			return tr("Subdivision Surfaces: Butterfly Subdivision");
		case FP_MIDPOINT                         : 
			return tr("Subdivision Surfaces: Midpoint");
		case FP_REFINE_CATMULL                   : 
			return tr("Subdivision Surfaces: Catmull-Clark");
		case FP_REMOVE_UNREFERENCED_VERTEX       : 
			return tr("Remove Unreferenced Vertices");
		case FP_REMOVE_DUPLICATED_VERTEX         :
			return tr("Remove Duplicate Vertices");
		case FP_REMOVE_FACE_ZERO_AREA            :
			return tr("Remove Zero Area Faces");
		case FP_QUADRIC_SIMPLIFICATION           :
			return tr("Simplification: Quadric Edge Collapse Decimation");
		case FP_QUADRIC_TEXCOORD_SIMPLIFICATION  : 
			return tr("Simplification: Quadric Edge Collapse Decimation (with texture)");
		case FP_CLUSTERING                       : 
			return tr("Simplification: Clustering Decimation");
		case FP_REORIENT                         : 
			return tr("Re-Orient all faces coherentely");
		case FP_INVERT_FACES                     : 
			return tr("Invert Faces Orientation");
		case FP_SCALE                            : 
			return QTextCodec::codecForName("GBK")->toUnicode("缩放"); /*tr("Transform: Scale, Normalize");*/
		case FP_CENTER                           :
			return QTextCodec::codecForName("GBK")->toUnicode("移动"); /*tr("Transform: Translate, Center, set Origin");*/
		case FP_ROTATE                           :
			return QTextCodec::codecForName("GBK")->toUnicode("旋转"); /*tr("Transform: Rotate");*/
		case FP_CLOSE_HOLES                      :
			return QTextCodec::codecForName("GBK")->toUnicode("补洞"); /*tr("Close Holes");*/
		case FP_ROTATE_FIT                       : 
			return tr("Transform: Rotate to Fit to a plane");
		case FP_PRINCIPAL_AXIS                   : 
			return tr("Transform: Align to Principal Axis");
		case FP_FLIP_AND_SWAP                    : 
			return tr("Transform: Flip and/or swap axis");
		case FP_FREEZE_TRANSFORM                 : 
			return tr("Matrix: Freeze Current Matrix");
		case FP_RESET_TRANSFORM                  : 
			return tr("Matrix: Reset Current Matrix");
		case FP_INVERT_TRANSFORM                 : 
			return tr("Matrix: Invert Current Matrix");
		case FP_SET_TRANSFORM_PARAMS             : 
			return tr("Matrix: Set from translation/rotation/scale");
		case FP_SET_TRANSFORM_MATRIX             : 
			return tr("Matrix: Set/Copy Transformation");
		case FP_NORMAL_EXTRAPOLATION             :
			return tr("Compute normals for point sets");
		case FP_NORMAL_SMOOTH_POINTCLOUD         : 
			return tr("Smooths normals on a point sets");
		case FP_COMPUTE_PRINC_CURV_DIR           : 
			return tr("Compute curvature principal directions");
		case FP_CYLINDER_UNWRAP                  : 
			return tr("Geometric Cylindrical Unwrapping");
		case FP_REFINE_HALF_CATMULL              : 
			return tr("Tri to Quad by 4-8 Subdivision");
		case FP_QUAD_DOMINANT                    :
			return tr("Turn into Quad-Dominant mesh");
		case FP_MAKE_PURE_TRI                    : 
			return tr("Turn into a Pure-Triangular mesh");
		case FP_QUAD_PAIRING                     : 
			return tr("Tri to Quad by smart triangle pairing");
		case FP_FAUX_CREASE                      : 
			return tr("Crease Marking with NonFaux Edges");
		case FP_FAUX_EXTRACT                     : 
			return tr("Build a Polyline with NonFaux Edges");
		case FP_VATTR_SEAM                       :
			return tr("Vertex Attribute Seam");
		case FP_REFINE_LS3_LOOP                  : 
			return tr("Subdivision Surfaces: LS3 Loop");
		case FP_SLICE_WITH_A_PLANE               :
			return tr("Compute Planar Section");
		case FP_PERIMETER_POLYLINE               : 
			return tr("Create Selection Perimeter Polyline");
...
QString ExtraMeshFilterPlugin::filterInfo(FilterIDType filterID) const
{
    ...
    {
		case FP_REFINE_LS3_LOOP                    : 
			return tr("Apply LS3 Subdivision Surface algorithm using Loop's weights. This refinement method take normals into account. "
															   "<br>See:"
		...
		case FP_LOOP_SS                            : 
			return tr("Apply Loop's Subdivision Surface algorithm. It is an approximant refinement method and it works for every triangle and has rules for extraordinary vertices.<br>");
		case FP_BUTTERFLY_SS                       : 
			return tr("Apply Butterfly Subdivision Surface algorithm. It is an interpolated refinement method, defined on arbitrary triangular meshes. The scheme is known to be C1 but not C2 on regular meshes<br>");
		case FP_MIDPOINT                           : 
			return tr("Apply a plain subdivision scheme where every edge is splitted on its midpoint. Useful to uniformly refine a mesh substituting each triangle with four smaller triangles.");
		case FP_REFINE_CATMULL                     : 
			return tr("Apply the Catmull-Clark Subdivision Surfaces. Note that position of the new vertices is simply linearly interpolated. If the mesh is triangle based (no faux edges) it generates a quad mesh, otherwise it honores it the faux-edge bits");
		case FP_REFINE_HALF_CATMULL                : 
			return tr("Convert a tri mesh into a quad mesh by applying a 4-8 subdivision scheme."
		...
		case FP_REMOVE_UNREFERENCED_VERTEX         : 
			return tr("Check for every vertex on the mesh: if it is NOT referenced by a face, removes it");
		case FP_REMOVE_DUPLICATED_VERTEX           : 
			return tr("Check for every vertex on the mesh: if there are two vertices with same coordinates they are merged into a single one.");
		case FP_CLUSTERING                         :
			return tr("Collapse vertices by creating a three dimensional grid enveloping the mesh and discretizes them based on the cells of this grid");
		case FP_QUADRIC_SIMPLIFICATION             :
			return tr("Simplify a mesh using a Quadric based Edge Collapse Strategy; better than clustering but slower");
		case FP_QUADRIC_TEXCOORD_SIMPLIFICATION    : 
			return tr("Simplify a textured mesh using a Quadric based Edge Collapse Strategy preserving UV parametrization; better than clustering but slower");
		case FP_REORIENT                           : 
			return tr("Re-orient in a consistent way all the faces of the mesh. <br>"
		...
		case FP_INVERT_FACES                       : 
			return tr("Invert faces orientation, flipping the normals of the mesh. <br>"
		...
		case FP_SCALE                              :
			return tr(""); /*tr("Generate a matrix transformation that scale the mesh. The mesh can be also automatically scaled to a unit side box. ");*/
		case FP_CENTER                             :
			return tr(""); /*tr("Generate a matrix transformation that translate the mesh. The mesh can be translated around one of the axis or a given axis and w.r.t. to the origin or the baricenter, or a given point.");*/
		case FP_ROTATE                             :
			return tr(""); /*tr("Generate a matrix transformation that rotates the mesh. The mesh can be rotated around one of the axis or a given axis and w.r.t. to the origin or the baricenter, or a given point.");*/
		case FP_CLOSE_HOLES                        :
			return tr(""); /*tr("Close holes smaller than a given threshold");*/
		case FP_ROTATE_FIT                         :
			return tr("Generate a matrix transformation that rotates the mesh so that the selection fits one of the main planes XY YZ ZX. May also translate such that the selection centroid rest on the origin. It reports on the log the average error of the fitting (in mesh units).");
		case FP_PRINCIPAL_AXIS                     :
			return tr("Generate a matrix transformation that rotates the mesh aligning it to its principal axis of inertia."
		...
		case FP_FLIP_AND_SWAP                      :
			return tr("Generate a matrix transformation that flips each one of the axis or swaps a couple of axis. The listed transformations are applied in that order. This kind of transformation cannot be applied to set of Raster!");
		case FP_RESET_TRANSFORM                    : 
			return tr("Set the current transformation matrix to the Identity. ");
		case FP_FREEZE_TRANSFORM                   :
			return tr("Freeze the current transformation matrix into the coordinates of the vertices of the mesh (and set this matrix to the identity). In other words it applies in a definetive way the current matrix to the vertex coordinates.");
		case FP_INVERT_TRANSFORM                   :
			return tr("Invert the current transformation matrix. The current transformation is reversed, becoming its opposite.");
		case FP_SET_TRANSFORM_PARAMS               : 
			return tr("Set the current transformation matrix starting from parameters: [XYZ] translation, [XYZ] Euler angles rotation and [XYZ] scaling.");
		case FP_SET_TRANSFORM_MATRIX               :
			return tr("Set the current transformation matrix by filling it, or copying from another layer.");
		case FP_NORMAL_EXTRAPOLATION               :
			return tr("Compute the normals of the vertices of a mesh without exploiting the triangle connectivity, useful for dataset with no faces");
		case FP_NORMAL_SMOOTH_POINTCLOUD           :
			return tr("Smooth the normals of the vertices of a mesh without exploiting the triangle connectivity, useful for dataset with no faces");
		case FP_COMPUTE_PRINC_CURV_DIR             : 
			return tr("Compute the principal directions of curvature with several algorithms");
		case FP_CYLINDER_UNWRAP                    :
			return tr("Unwrap the geometry of current mesh along a clylindrical equatorial projection. The cylindrical projection axis is centered on the origin and directed along the vertical <b>Y</b> axis.");
		case FP_QUAD_PAIRING                       : 
			return tr("Convert into a tri-mesh into a quad mesh by pairing triangles.");
		case FP_QUAD_DOMINANT                      : 
			return tr("Convert into a tri-mesh into a quad-dominant mesh by pairing suitable triangles.");
		case FP_MAKE_PURE_TRI                      : 
			return tr("Convert into a tri-mesh by splitting any polygonal face.");
		case FP_FAUX_CREASE                        : 
			return tr("Mark the crease edges of a mesh as Non-Faux according to edge dihedral angle.<br>"
		...
		case FP_VATTR_SEAM                         :
			return tr("Make all selected vertex attributes connectivity-independent:<br/>"
		...
		case FP_SLICE_WITH_A_PLANE                 : 
			return tr("Compute the polyline representing a planar section (a slice) of a mesh; if the resulting polyline is closed the result is filled and also a triangular mesh representing the section is saved");
		case FP_PERIMETER_POLYLINE                 : 
			return tr("Create a new Layer with the perimeter polyline(s) of the selection borders");
		case FP_FAUX_EXTRACT                       :
			return tr("Create a new Layer with an edge mesh composed only by the non faux edges of the current mesh");
		case FP_REMOVE_FACE_ZERO_AREA              :
			return tr("Remove null faces (the one with area equal to zero)");
...
void ExtraMeshFilterPlugin::initParameterSet(QAction * action, MeshModel & m, RichParameterSet & parlst)
{
    ...
    switch(ID(action))
    {
		...
		case FP_ROTATE:
		{
			...
			rotMethod.push_back(QTextCodec::codecForName("GBK")->toUnicode("X轴") /*"X axis"*/);
			rotMethod.push_back(QTextCodec::codecForName("GBK")->toUnicode("Y轴") /*"Y axis"*/);
			rotMethod.push_back(QTextCodec::codecForName("GBK")->toUnicode("Z轴") /*"Z axis"*/);
			rotMethod.push_back(QTextCodec::codecForName("GBK")->toUnicode("定制轴") /*"custom axis"*/);
			parlst.addParam(new RichEnum("rotAxis", 0, rotMethod, QTextCodec::codecForName("GBK")->toUnicode("旋转在:"), "" /*tr("Rotation on:"), tr("Choose a method")*/));

			QStringList rotCenter;
			rotCenter.push_back(QTextCodec::codecForName("GBK")->toUnicode("原点") /*"origin"*/);
			rotCenter.push_back(QTextCodec::codecForName("GBK")->toUnicode("重心") /*"barycenter"*/);
			rotCenter.push_back(QTextCodec::codecForName("GBK")->toUnicode("定制点") /*"custom point"*/);
			parlst.addParam(new RichEnum("rotCenter", 0, rotCenter, QTextCodec::codecForName("GBK")->toUnicode("旋转中心: "), "" /*tr("Center of rotation:"), tr("Choose a method")*/));
			parlst.addParam(new RichDynamicFloat("angle",0,-360,360, QTextCodec::codecForName("GBK")->toUnicode("旋转角度: "), "" /*"Rotation Angle","Angle of rotation (in <b>degree</b>). If snapping is enable this vaule is rounded according to the snap value"*/));
			parlst.addParam(new RichPoint3f("customAxis",Point3f(0,0,0), QTextCodec::codecForName("GBK")->toUnicode("定制轴: "), "" /*"Custom axis","This rotation axis is used only if the 'custom axis' option is chosen."*/));
			parlst.addParam(new RichPoint3f("customCenter",Point3f(0,0,0), QTextCodec::codecForName("GBK")->toUnicode("定制中心: "), "" /*"Custom center","This rotation center is used only if the 'custom point' option is chosen."*/));
			parlst.addParam(new RichBool("snapFlag",false, QTextCodec::codecForName("GBK")->toUnicode("捕捉角度"), "" /*"Snap angle","If selected, before starting the filter will remove anyy unreference vertex (for which curvature values are not defined)"*/));
			parlst.addParam(new RichFloat("snapAngle",30, QTextCodec::codecForName("GBK")->toUnicode("俘获值: "), "" /*"Snapping Value","This value is used to snap the rotation angle."*/));
			parlst.addParam(new RichBool ("Freeze",true, QTextCodec::codecForName("GBK")->toUnicode("冻结矩阵"), "" /*"Freeze Matrix","The transformation is explicitly applied and the vertex coords are actually changed"*/));
			parlst.addParam(new RichBool ("allLayers",false, QTextCodec::codecForName("GBK")->toUnicode("应用于所有层"), "" /*"Apply to all layers","The transformation is explicitly applied to all the mesh and raster layers in the project"*/));
			break;
		} 
		...
		case FP_CENTER:
		{
			...
			traslMethod.push_back(QTextCodec::codecForName("GBK")->toUnicode("XYZ平移") /*"XYZ translation"*/);
			traslMethod.push_back(QTextCodec::codecForName("GBK")->toUnicode("边框中心") /*"Center on BBox"*/);
			traslMethod.push_back(QTextCodec::codecForName("GBK")->toUnicode("设置新原点") /*"Set new Origin"*/);
			parlst.addParam(new RichEnum("traslMethod", 0, traslMethod, QTextCodec::codecForName("GBK")->toUnicode("移动:"), "" /*tr("Transformation:"), tr("[XYZ translation] adds X,Y and Z offset to Layer transformation, [Center on BBox] moves Layer Origin to the Bounding Box center, [Set new Origin] moves Layer Origin to a specific point")*/));
			
			Box3m &bb=m.cm.bbox;
			parlst.addParam(new RichDynamicFloat("axisX",0,-5.0*bb.Diag(),5.0*bb.Diag(), QTextCodec::codecForName("GBK")->toUnicode("X轴: "), "" /*"X Axis","Absolute translation amount along the X axis"*/));
			parlst.addParam(new RichDynamicFloat("axisY",0,-5.0*bb.Diag(),5.0*bb.Diag(), QTextCodec::codecForName("GBK")->toUnicode("Y轴: "), "" /*"Y Axis","Absolute translation amount along the Y axis"*/));
			parlst.addParam(new RichDynamicFloat("axisZ",0,-5.0*bb.Diag(),5.0*bb.Diag(), QTextCodec::codecForName("GBK")->toUnicode("Z轴: "), "" /*"Z Axis","Absolute translation amount along the Z axis"*/));
			parlst.addParam(new RichPoint3f("newOrigin", Point3f(0, 0, 0), QTextCodec::codecForName("GBK")->toUnicode("新原点: "), "" /*"New Origin:", "when using [Set new Origin], this is the location of the new Origin."*/));
			parlst.addParam(new RichBool ("Freeze", true, QTextCodec::codecForName("GBK")->toUnicode("冻结矩阵"), "" /*"Freeze Matrix","The transformation is explicitly applied and the vertex coords are actually changed"*/));
			parlst.addParam(new RichBool ("allLayers", false, QTextCodec::codecForName("GBK")->toUnicode("应用于所有层"), "" /*"Apply to all layers","The transformation is explicitly applied to all the mesh and raster layers in the project"*/));
			break;
		} 
		...
		case FP_SCALE:
		{
			parlst.addParam(new RichFloat("axisX",1, QTextCodec::codecForName("GBK")->toUnicode("X轴: "), "" /*"X Axis","Scaling"*/));
			parlst.addParam(new RichFloat("axisY",1, QTextCodec::codecForName("GBK")->toUnicode("Y轴: "), "" /*"Y Axis","Scaling"*/));
			parlst.addParam(new RichFloat("axisZ",1, QTextCodec::codecForName("GBK")->toUnicode("Z轴: "), "" /*"Z Axis","Scaling"*/));
			parlst.addParam(new RichBool("uniformFlag",true, QTextCodec::codecForName("GBK")->toUnicode("统一缩放:"), "" /*"Uniform Scaling","If selected an uniform scaling (the same for all the three axis) is applied (the X axis value is used)"*/));

			QStringList scaleCenter;
			scaleCenter.push_back(QTextCodec::codecForName("GBK")->toUnicode("原点") /*"origin"*/);
			scaleCenter.push_back(QTextCodec::codecForName("GBK")->toUnicode("重心") /*"barycenter"*/);
			scaleCenter.push_back(QTextCodec::codecForName("GBK")->toUnicode("定制点") /*"custom point"*/);
			parlst.addParam(new RichEnum("scaleCenter", 0, scaleCenter, QTextCodec::codecForName("GBK")->toUnicode("缩放中心:"), "" /*tr("Center of scaling:"), tr("Choose a method"*/));
			parlst.addParam(new RichPoint3f("customCenter", Point3f(0,0,0), QTextCodec::codecForName("GBK")->toUnicode("定制中心:"), "" /*"Custom center","This scaling center is used only if the 'custom point' option is chosen."*/));
			parlst.addParam(new RichBool("unitFlag", false, QTextCodec::codecForName("GBK")->toUnicode("在单位盒内缩放"), "" /*"Scale to Unit bbox","If selected, the object is scaled to a box whose sides are at most 1 unit lenght"*/));
			parlst.addParam(new RichBool ("Freeze", true, QTextCodec::codecForName("GBK")->toUnicode("冻结矩阵"), "" /*"Freeze Matrix","The transformation is explicitly applied and the vertex coords are actually changed"*/));
			parlst.addParam(new RichBool ("allLayers" ,false, QTextCodec::codecForName("GBK")->toUnicode("应用于所有层"), "" /*"Apply to all layers","The transformation is explicitly applied to all the mesh and raster layers in the project"*/));
			break;
		} 
		...
		case FP_CLOSE_HOLES:
		{
			parlst.addParam(new RichInt ("MaxHoleSize", (int)30, QTextCodec::codecForName("GBK")->toUnicode("补洞大小:"), "" /*"Max size to be closed ","The size is expressed as number of edges composing the hole boundary"*/));
			parlst.addParam(new RichBool("Selected", m.cm.sfn>0, QTextCodec::codecForName("GBK")->toUnicode("在选中的面中补洞"), "" /*"Close holes with selected faces","Only the holes with at least one of the boundary faces selected are closed"*/));
			// Delete some property by Johnny Xu, 2017/6/28
			//parlst.addParam(new RichBool("NewFaceSelected", true, QTextCodec::codecForName("GBK")->toUnicode("选择最新创建的面"), "" /* "Select the newly created faces","After closing a hole the faces that have been created are left selected. Any previous selection is lost. Useful for example for smoothing the newly created holes."*/));
			//parlst.addParam(new RichBool("SelfIntersection", true, QTextCodec::codecForName("GBK")->toUnicode("防止自相交面的创建"), "" /*"Prevent creation of selfIntersecting faces","When closing an holes it tries to prevent the creation of faces that intersect faces adjacent to the boundary of the hole. It is an heuristic, non intersetcting hole filling can be NP-complete."*/));
			break;
		}	
...
bool ExtraMeshFilterPlugin::applyFilter(QAction * filter, MeshDocument & md, RichParameterSet & par, vcg::CallBackPos * cb)
{
	...
	switch(ID(filter))
	{
		...
		case FP_ROTATE :
		{
			...
			switch(par.getEnum("rotCenter"))
			{
				case 0: 
					...
					break;
				case 1: 
					tranVec= m.cm.Tr * m.cm.bbox.Center();
					break;
				case 2: 
		...
		case FP_CENTER:
		{
			...
			{
				case 0: 
					break; //we already got it from interface
				case 1: 
					translation = -(m.cm.Tr * md.bbox().Center()); 
					break; // we consider current transformation when finding bbox center
				case 2: 
					translation = -par.getPoint3m("newOrigin"); 
					break;
			}
		...
		case FP_SCALE:
		{
			...
			switch (par.getEnum("scaleCenter"))
			{
				case 0: 
					...
					break;
				case 1:
					tranVec = m.cm.Tr * scalebb.Center(); 
					break;
				case 2: 
					tranVec = par.getPoint3m("customCenter"); 
					break;
		...
		case FP_CLOSE_HOLES:
		{
			...
			if (tri::Clean<CMeshO>::CountNonManifoldEdgeFF(m.cm) > 0)
			{
				errorMessage = QTextCodec::codecForName("GBK")->toUnicode("网格中有一些没有二维流型边缘。"); /*"Mesh has some not 2-manifold edges, filter require edge manifoldness"*/;
				return false;
			}
			...
			// Delete some property by Johnny Xu, 2017/6/28
			bool SelfIntersectionFlag = true; /*par.getBool("SelfIntersection");*/
			bool NewFaceSelectedFlag = true;  /*par.getBool("NewFaceSelected");*/
			...
			Log("填补了：%i个洞，新增了：%i个新面。", holeCnt, m.cm.fn-OriginalSize);
...
int ExtraMeshFilterPlugin::postCondition(QAction * filter) const
{
    switch (ID(filter))
    {
		...
		case FP_SET_TRANSFORM_MATRIX :
		case FP_FREEZE_TRANSFORM     : 
			return MeshModel::MM_TRANSFMATRIX + MeshModel::MM_VERTCOORD + MeshModel::MM_VERTNORMAL;
		case FP_RESET_TRANSFORM      :
		case FP_INVERT_TRANSFORM     : 
