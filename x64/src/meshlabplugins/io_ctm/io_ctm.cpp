...
bool IOMPlugin::save(const QString &/*formatName*/, const QString &fileName, MeshModel &m, const int mask,const RichParameterSet & par,  vcg::CallBackPos *cb, QWidget *parent)
{
    ...
    if(result!=0)
    {
		...
		QString strBox = errorMsgFormat.arg(qPrintable(fileName), vcg::tri::io::ExporterCTM<CMeshO>::ErrorMsg(result));
		QMessageBox box(QMessageBox::Warning, QString::fromLocal8Bit("±£´æ´íÎó"), strBox);
