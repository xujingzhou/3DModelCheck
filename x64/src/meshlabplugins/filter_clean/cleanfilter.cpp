...
CleanFilter::CleanFilter()
{
  ...
  /*typeList
	...
	// Commented by Johnny Xu, 2017/2/20
...
QString CleanFilter::filterName(FilterIDType filter) const
{
  switch(filter)
  {
	  case FP_BALL_PIVOTING :					
		  return QString("Surface Reconstruction: Ball Pivoting");
	  case FP_REMOVE_WRT_Q :            		
		  return QString("Remove Vertices wrt Quality");
	  case FP_REMOVE_ISOLATED_DIAMETER   :	
		  return QString("Remove Isolated pieces (wrt Diameter)");
	  case FP_REMOVE_ISOLATED_COMPLEXITY :	
		  return QTextCodec::codecForName("GBK")->toUnicode("去除孤立面") /*QString("Remove Isolated pieces (wrt Face Num.)")*/; 
	  case FP_REMOVE_TVERTEX_FLIP :          
		  return QString("Remove T-Vertices by Edge Flip");
	  case FP_REMOVE_TVERTEX_COLLAPSE :       
		  return QString("Remove T-Vertices by Edge Collapse");
	  case FP_SNAP_MISMATCHED_BORDER :      
		  return QString("Snap Mismatched Borders");
	  case FP_MERGE_CLOSE_VERTEX :          
		  return QString("Merge Close Vertices");
	  case FP_REMOVE_DUPLICATE_FACE:        
		  return QString("Remove Duplicate Faces");
	  case FP_REMOVE_FOLD_FACE:              
		  return QString("Remove Isolated Folded Faces by Edge Flip");
	  case FP_REMOVE_NON_MANIF_EDGE:        
		  return QString("Remove Faces from Non Manifold Edges");
	  case FP_REMOVE_NON_MANIF_VERT:       
		  return QString("Split Vertexes Incident on Non Manifold Faces");
	  case FP_COMPACT_VERT:					
		  return QString("Compact vertices");
	  case FP_COMPACT_FACE:					
		  return QString("Compact faces");
	  default: 
		  assert(0);
  }
 ...
 QString CleanFilter::filterInfo(FilterIDType filterId) const
{
	...
	{
		case FP_BALL_PIVOTING :	
			return QString("Given a point cloud with normals it reconstructs a surface using the <b>Ball Pivoting Algorithm</b>."
													...
													"IEEE TVCG 1999");
		case FP_REMOVE_ISOLATED_COMPLEXITY: 
			return QString(""); /*QString("Delete isolated connected components composed by a limited number of triangles")*/;
		case FP_REMOVE_ISOLATED_DIAMETER:  
			return QString("Delete isolated connected components whose diameter is smaller than the specified constant");
		case FP_REMOVE_WRT_Q:              
			return QString("Delete all the vertices with a quality lower smaller than the specified constant");
		case FP_REMOVE_TVERTEX_COLLAPSE :  
			return QString("Delete t-vertices from the mesh by collapsing the shortest of the incident edges");
		case FP_REMOVE_TVERTEX_FLIP :     
			return QString("Delete t-vertices by flipping the opposite edge on the degenerate face if the triangulation quality improves");
		case FP_SNAP_MISMATCHED_BORDER :   
			return QString("Try to snap together adjacent borders that are slightly mismatched.<br>"
		...
		case FP_MERGE_CLOSE_VERTEX :      
			return QString("Merge together all the vertices that are nearer than the speicified threshold. Like a unify duplicated vertices but with some tolerance.");
		case FP_REMOVE_DUPLICATE_FACE :  
			return QString("Delete all the duplicate faces. Two faces are considered equal if they are composed by the same set of verticies, regardless of the order of the vertices.");
		case FP_REMOVE_FOLD_FACE :      
			return QString("Delete all the single folded faces. A face is considered folded if its normal is opposite to all the adjacent faces. It is removed by flipping it against the face f adjacent along the edge e such that the vertex opposite to e fall inside f");
		case FP_REMOVE_NON_MANIF_EDGE :   
			return QString("For each non manifold edge it iteratively deletes the smallest area face until it becomes 2-manifold.");
		case FP_REMOVE_NON_MANIF_VERT :  
			return QString("Split non manifold vertices until it becomes 2-manifold.");
		case FP_COMPACT_VERT:           
			return QString("Compact all the vertices that have been deleted and put them to the end of the vector");
		case FP_COMPACT_FACE:         
			return QString("Compact all the faces that have been deleted and put them to the end of the vector");
		default: 
			assert(0);
	}
...
void CleanFilter::initParameterSet(QAction *action,MeshDocument &md, RichParameterSet & parlst)
{
	...
	switch(ID(action))
	{
		...
		case FP_REMOVE_ISOLATED_DIAMETER:
				...
				break;
		case FP_REMOVE_ISOLATED_COMPLEXITY:
				parlst.addParam(new RichInt("MinComponentSize", (int)minCC, QTextCodec::codecForName("GBK")->toUnicode("最小部件大小:"), ""  /*"Enter minimum conn. comp size:","Delete all the connected components (floating pieces) composed by a number of triangles smaller than the specified one"*/));
				parlst.addParam(new RichBool("removeUnref", true, QTextCodec::codecForName("GBK")->toUnicode("删除未引用点"), "" /*"Remove unfreferenced vertices", "if true, the unreferenced vertices remaining after the face deletion are removed."*/));
				break;
		case FP_REMOVE_WRT_Q:
...
bool CleanFilter::applyFilter(QAction *filter, MeshDocument &md, RichParameterSet & par, vcg::CallBackPos * cb)
{
    ...
    switch(ID(filter))
    {
		...
		case FP_REMOVE_ISOLATED_COMPLEXITY:
		{
			...
			Log("从%i个部件中，删除了%i个连接部件", delInfo.first, delInfo.second);

			if (par.getBool("removeUnref"))
			{
				int delvert = tri::Clean<CMeshO>::RemoveUnreferencedVertex(m.cm);
				Log("删除了%d个未引用点", delvert);
			}
		}
		break;
...
int SnapVertexBorder(CMeshO &m, float threshold, vcg::CallBackPos * cb)
{
  ...
  for(CMeshO::VertexIterator vi=m.vert.begin();vi!=m.vert.end();++vi)
    if((*vi).IsB())
      {
		 // Modified by Johnny Xu, 2017/2/20
        cb((tri::Index(m,*vi) *100) / m.vn, qPrintable(QTextCodec::codecForName("GBK")->toUnicode("抓取点集")));
