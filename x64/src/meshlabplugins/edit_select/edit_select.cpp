...
{
	return tr(""); /*tr("Interactive selection of faces inside a dragged rectangle in screen space");*/
}
...
void EditSelectPlugin::Decorate(MeshModel &m, GLArea * gla)
{
	if (isDragging)
	{
		...
		else
		{
			//m.cm.selface.clear();
			if (selectFrontFlag)	
				GLPickTri<CMeshO>::PickVisibleFace(mid[0], mid[1], m.cm, NewSelFace, wid[0], wid[1]);
			else                
				GLPickTri<CMeshO>::PickFace(mid[0], mid[1], m.cm, NewSelFace, wid[0], wid[1]);
