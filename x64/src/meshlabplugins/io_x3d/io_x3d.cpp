...
bool IoX3DPlugin::open(const QString &formatName, const QString &fileName, MeshModel &m, int& mask, const RichParameterSet &, CallBackPos *cb, QWidget *parent)
{
    ...
    if(formatName.toUpper() == tr("X3D") || formatName.toUpper() == tr("X3DV") || formatName.toUpper() == tr("WRL"))
    {
		...
		{
			QMessageBox box(QMessageBox::Critical, QString::fromLocal8Bit("打开错误"), errorMsgFormat.arg(fileName, info->filenameStack[info->filenameStack.size() - 1], vcg::tri::io::ImporterX3D<CMeshO>::ErrorMsg(result)));
        ...
        if (result != vcg::tri::io::ImporterX3D<CMeshO>::E_NOERROR)
        {
			...
			QMessageBox box(QMessageBox::Critical, QString::fromLocal8Bit("打开错误"), errorMsgFormat.arg(fileName, fileError, lineError, vcg::tri::io::ImporterX3D<CMeshO>::ErrorMsg(result)));
			box.setStandardButtons(QMessageBox::Ok);
			box.setButtonText(QMessageBox::Ok, QString::fromLocal8Bit("确 定"));
			box.exec();
			delete info;
        ...
        if (m.cm.vert.size() == 0)
        {
			...
			QMessageBox box(QMessageBox::Critical, QString::fromLocal8Bit("打开错误"), errorMsgFormat.arg(fileName));
			box.setStandardButtons(QMessageBox::Ok);
			box.setButtonText(QMessageBox::Ok, QString::fromLocal8Bit("确 定"));
			box.exec();
			delete info;
		...
		if (someTextureNotFound)
		{
			...
			QMessageBox box(QMessageBox::Warning, QString::fromLocal8Bit("文件缺失"), missingTextureFilesMsg);
			box.setStandardButtons(QMessageBox::Ok);
			box.setButtonText(QMessageBox::Ok, QString::fromLocal8Bit("确 定"));
			box.exec();
		}
...
bool IoX3DPlugin::save(const QString &formatName, const QString &fileName, MeshModel &m, const int mask, const RichParameterSet &, vcg::CallBackPos *cb, QWidget *parent)
{
    ...
    if(formatName.toUpper() == tr("X3D"))
    {
        ...
        if(result!=0)
		{
			QString strBox = errorMsgFormat.arg(fileName, vcg::tri::io::ExporterX3D<CMeshO>::ErrorMsg(result));
